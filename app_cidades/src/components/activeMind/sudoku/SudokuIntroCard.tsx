...
type SudokuIntroCardProps = {
  game: ActiveMindGame
  onHowToPlayPress: () => void
}

export function SudokuIntroCard({ game, onHowToPlayPress }: SudokuIntroCardProps) {
    ...
    <View style={styles.card}>
      <View style={styles.topRow}>
        ...
        <View style={styles.textCol}>
          <Text style={styles.title}>{SUDOKU_INTRO.title}</Text>
          <Text style={styles.summary}>{SUDOKU_INTRO.summary}</Text>
        </View>
      </View>

      <Text style={styles.rules}>{SUDOKU_INTRO.rulesBrief}</Text>

      <Pressable
        onPress={handleHowToPlayPress}
        style={({ pressed }) => [styles.linkWrap, pressed && styles.linkWrapPressed]}
        accessibilityRole="button"
        accessibilityLabel="Como jogar Sudoku"
      >
