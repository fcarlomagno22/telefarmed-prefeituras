...
type SleepTimeMonthPickerDrawerProps = {
  visible: boolean
  monthKey: string
  onClose: () => void
...
export function SleepTimeMonthPickerDrawer({
  visible,
  monthKey,
  onClose,
  onApply,
}: SleepTimeMonthPickerDrawerProps) {
  ...
  const footerHint = useMemo(() => `Visualizando: ${getEatWellMonthLabel(draftMonthKey)}`, [draftMonthKey])
  ...
  return (
    <RunWalkSheetDrawer
      visible={visible}
      title="Escolher mês"
      subtitle="Navegue pelo histórico de sono mês a mês"
      minHeight="48%"
      hideCloseButton
      extraBottomInset={28}
      onClose={onClose}
    ...
    >
      <View style={styles.yearRow}>
        ...
        >
          <Ionicons name="chevron-forward" size={18} color={canGoNextYear ? colors.text : colors.textSubtle} />
        </Pressable>
      </View>
      ...
      <View style={styles.monthGrid}>
        {MONTH_LABELS.map((label, monthIndex) => {
          ...
          const isCurrent = formatMonthKey(draftYear, monthIndex) === currentMonthKey
