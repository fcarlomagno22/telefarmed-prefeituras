...
import { MEAL_LOG_WIZARD_STEPS } from '../../utils/eatWellMealLogWizard'
import { colors } from '../../theme/colors'

const DOT_SIZE = 34

type EatWellMealLogTimelineProps = {
  currentStep: number
}
...
export function EatWellMealLogTimeline({ currentStep }: EatWellMealLogTimelineProps) {
  return (
    <View style={styles.wrapper}>
      <View style={styles.timeline}>
        {MEAL_LOG_WIZARD_STEPS.map((step, index) => {
          const isCompleted = step.id < currentStep
          const isCurrent = step.id === currentStep
          const isUpcoming = step.id > currentStep
          const isFirst = index === 0
          const isLast = index === MEAL_LOG_WIZARD_STEPS.length - 1
