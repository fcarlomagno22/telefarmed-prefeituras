...
export function EatWellMenusTab({
  ...
  const [menuPendingDelete, setMenuPendingDelete] = useState<EatWellSavedMenu | null>(null)
  ...
  function handleConfirmDelete() {
    ...
    onDeleteMenu(menuPendingDelete.id)
  ...
  return (
    <>
            ...
            <View style={styles.list}>
              {menus.map((menu) => (
                <EatWellMenuCard
                  ...
                  onDeletePress={() => setMenuPendingDelete(menu)}
                />
              ))}
            </View>
          </>
      ...
      <DeleteConfirmSheet
        visible={menuPendingDelete !== null}
        title="Excluir cardápio?"
        message={
          menuPendingDelete
            ? `Deseja excluir "${menuPendingDelete.name}"? Esta ação não pode ser desfeita.`
            : ''
        }
        confirmLabel="Excluir cardápio"
