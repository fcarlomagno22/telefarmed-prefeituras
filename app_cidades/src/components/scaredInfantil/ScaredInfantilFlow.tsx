...
import { saveScaredSession } from '../../data/scaredInfantilStorage'
import { runScaredEngine } from '../../scaredInfantil/engine'
import {
  buildScaredFlowQuestions,
  getScaredConsentDisclaimer,
  getScaredConsentItems,
  getScaredDataCollectionNotice,
  getScaredInformantOptions,
  getScaredStepLabel,
  resolveScaredRespondentId,
  SCARED_FLOW_STEPS,
} from '../../scaredInfantil/flowBuilder'
import type {
  ScaredAnswers,
  ScaredConsentItem,
  ScaredEngineResult,
...
function createSessionId() {
  return `scared_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`
}
    ...
    <View style={styles.consentRow}>
      ...
      >
        <Ionicons
          ...
          size={20}
          color={accepted ? '#86efac' : colors.textSubtle}
        />
      </Pressable>
...
export function ScaredInfantilFlow({
  visible,
  patientCpf,
  onClose,
  onCompleted,
}: ScaredInfantilFlowProps) {
  const insets = useSafeAreaInsets()
  const consentItems = useMemo(() => getScaredConsentItems(), [])
  const consentDisclaimer = useMemo(() => getScaredConsentDisclaimer(), [])
  const dataCollectionNotice = useMemo(() => getScaredDataCollectionNotice(), [])

  const [stepIndex, setStepIndex] = useState(0)
  const [acceptedConsents, setAcceptedConsents] = useState<Record<string, boolean>>({})
  const [activeConsentItem, setActiveConsentItem] = useState<ScaredConsentItem | null>(null)
  const [dataNoticeVisible, setDataNoticeVisible] = useState(false)
  const [childName, setChildName] = useState('')
  const [childAgeYears, setChildAgeYears] = useState('')
  const [informantTypeId, setInformantTypeId] = useState<ScaredInformantTypeId | null>(null)
  const [respondentId, setRespondentId] = useState<ScaredRespondentId>('responsavel')
  const [answers, setAnswers] = useState<ScaredAnswers>({})
  const [questionIndex, setQuestionIndex] = useState(0)
  const [result, setResult] = useState<ScaredEngineResult | null>(null)
  const [isSaving, setIsSaving] = useState(false)

  const parsedAge = Number(childAgeYears.replace(/\D/g, ''))
  const informantOptions = useMemo(
    () => (parsedAge >= 4 && parsedAge <= 17 ? getScaredInformantOptions(parsedAge) : []),
    [parsedAge],
  )
  ...
  const currentStep = SCARED_FLOW_STEPS[stepIndex] ?? 'consent'
  const flowQuestions = useMemo(
    () => (parsedAge > 0 ? buildScaredFlowQuestions(parsedAge, respondentId) : []),
    [parsedAge, respondentId],
  )
  ...
  const isQuestionStep = ['scared', 'functional', 'red_flags', 'differential'].includes(currentStep)
  ...
  const goNextStep = useCallback(() => {
    setQuestionIndex(0)
    setStepIndex((current) => Math.min(current + 1, SCARED_FLOW_STEPS.length))
  }, [])

  const finalize = useCallback(
    async (nextAnswers: ScaredAnswers) => {
      if (!informantTypeId || !parsedAge) return
      const engineResult = runScaredEngine(nextAnswers, {
      ...
      setIsSaving(true)
      try {
        await saveScaredSession(patientCpf, {
          id: createSessionId(),
          childName: childName.trim() || undefined,
          childAgeYears: parsedAge,
          informantTypeId,
    ...
    [acceptedConsents, childName, informantTypeId, onCompleted, parsedAge, patientCpf, respondentId],
  )
  ...
  const handleContinue = useCallback(() => {
    ...
    if (isQuestionStep) {
      ...
      if (stepIndex < SCARED_FLOW_STEPS.length - 1) {
        goNextStep()
        return
      }
  ...
  const handleInformantSelect = useCallback(
    (id: ScaredInformantTypeId) => {
      void Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light)
      setInformantTypeId(id)
      setRespondentId(resolveScaredRespondentId(id))
      ...
      <View style={[styles.root, { paddingTop: Math.max(insets.top, 12) }]}>
        <View style={styles.header}>
          ...
          <View style={styles.headerText}>
            <Text style={styles.headerTitle}>Ansiedade infantil</Text>
            <Text style={styles.headerSubtitle}>
              {result ? 'Resultado' : getScaredStepLabel(currentStep)}
            </Text>
          </View>
        </View>
        ...
        {!result ? (
          <KeyboardAvoidingView style={styles.body} behavior={keyboardAvoidingBehavior}>
            ...
            >
              ...
              {currentStep === 'informant' ? (
                <View style={styles.section}>
                  <Text style={styles.lead}>Quem está respondendo agora?</Text>
                  {informantOptions.map((option) => {
                    const selected = informantTypeId === option.id
                    return (
                      <Pressable
                        key={option.id}
                        onPress={() => handleInformantSelect(option.id)}
                      ...
                      >
                        ...
                        {option.hint ? (
                          <Text style={[styles.choiceHint, selected && styles.choiceTextSelected]}>
                            {option.hint}
                          </Text>
                        ) : null}
                      </Pressable>
          ...
          <ScaredInfantilResultView result={result} onClose={handleClose} />
        )}
      </View>
    </AppModal>

      <ScaredConsentTermDrawer
