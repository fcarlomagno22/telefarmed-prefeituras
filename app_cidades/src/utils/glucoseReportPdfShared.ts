import { createPdfFileFromHtml, type PdfFileResult } from '../adapters/pdfDocument'
...
export function buildGlucoseReportFilename(): string {
  const stamp = new Date().toISOString().slice(0, 10)
  return `relatorio-glicemia-${stamp}.pdf`
}

export async function createGlucoseReportPdf(
  ...
  return createPdfFileFromHtml({ html })
}
