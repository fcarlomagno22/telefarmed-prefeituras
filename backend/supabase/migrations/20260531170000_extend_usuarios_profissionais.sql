-- Etapa 2.3 — Perfil completo de profissionais ativos
...
COMMENT ON COLUMN usuarios_profissionais.dados_pj IS
  'JSON: cnpj, razaoSocial, municipio, uf, bancoNome, bancoCodigo, agencia, conta, tipoConta, pixTipo, pixChave.';
COMMENT ON COLUMN usuarios_profissionais.assinatura IS
  'JSON certificado ICP-Brasil: modo, status, updatedAt, expiresAt, emissorDescricao, arquivoNome, titularNome.';
COMMENT ON COLUMN usuarios_profissionais.foto_storage_path IS
  'Path no bucket profissionais-fotos (URL assinada via backend).';
...
-- Backfill a partir de candidaturas aprovadas/finalizadas
