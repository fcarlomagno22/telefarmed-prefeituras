-- Reaplica tabelas de anotações e registros de contato do drawer UBT
...
CREATE INDEX IF NOT EXISTS paciente_anotacoes_paciente_idx
  ON paciente_anotacoes (paciente_id, criado_em DESC);

CREATE INDEX IF NOT EXISTS paciente_anotacoes_entidade_idx
  ON paciente_anotacoes (entidade_contratante_id, criado_em DESC);

CREATE TABLE IF NOT EXISTS paciente_registros_contato (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  paciente_id UUID NOT NULL REFERENCES pacientes(id) ON DELETE CASCADE,
  entidade_contratante_id UUID NOT NULL REFERENCES entidades_contratantes(id) ON DELETE RESTRICT,
  unidade_ubt_id UUID NOT NULL REFERENCES unidades_ubt(id) ON DELETE RESTRICT,
...
CREATE INDEX IF NOT EXISTS paciente_registros_contato_paciente_idx
