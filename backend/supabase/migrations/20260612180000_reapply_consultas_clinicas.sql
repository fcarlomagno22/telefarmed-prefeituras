-- Reapply consultas clínicas (idempotente) — triagem UBT + telemedicina

DO $$ BEGIN
  CREATE TYPE consulta_status AS ENUM (
    'aguardando_medico', 'em_andamento', 'concluida', 'cancelada', 'interrompida'
  );
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN
  CREATE TYPE consulta_tipo AS ENUM ('consulta', 'retorno', 'primeira_consulta');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN
  CREATE TYPE consulta_mensagem_remetente AS ENUM ('paciente', 'profissional', 'sistema');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN
  CREATE TYPE consulta_anexo_tipo AS ENUM (
    'receita', 'pedido_exame', 'cardapio', 'plano_alimentar', 'orientacao',
    'atestado', 'encaminhamento', 'outro'
  );
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

DO $$ BEGIN
  CREATE TYPE consulta_anexo_origem AS ENUM ('paciente', 'profissional');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
CREATE TABLE IF NOT EXISTS consultas (
  ...
  sala_espera_entrada_em TIMESTAMPTZ,
  contabilizada_em TIMESTAMPTZ,
...
CREATE UNIQUE INDEX IF NOT EXISTS consultas_codigo_atendimento_uidx ON consultas (codigo_atendimento);
CREATE INDEX IF NOT EXISTS consultas_unidade_status_idx ON consultas (unidade_ubt_id, status, criado_em DESC);
CREATE INDEX IF NOT EXISTS consultas_entidade_criado_idx ON consultas (entidade_contratante_id, criado_em DESC);
CREATE INDEX IF NOT EXISTS consultas_profissional_finalizada_idx
  ON consultas (profissional_id, finalizada_em DESC) WHERE profissional_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS consultas_paciente_idx ON consultas (paciente_id);
CREATE INDEX IF NOT EXISTS consultas_status_criado_idx ON consultas (status, criado_em);

CREATE TABLE IF NOT EXISTS consulta_mensagens (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX IF NOT EXISTS consulta_mensagens_consulta_idx ON consulta_mensagens (consulta_id, enviada_em);

CREATE TABLE IF NOT EXISTS consulta_prescricoes (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  consulta_id UUID NOT NULL REFERENCES consultas(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS consulta_prescricoes_consulta_idx ON consulta_prescricoes (consulta_id);

CREATE TABLE IF NOT EXISTS consulta_solicitacoes_exame (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  consulta_id UUID NOT NULL REFERENCES consultas(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS consulta_solicitacoes_exame_consulta_idx ON consulta_solicitacoes_exame (consulta_id);

CREATE TABLE IF NOT EXISTS consulta_avaliacoes (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS consulta_anexos (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  consulta_id UUID NOT NULL REFERENCES consultas(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS consulta_anexos_consulta_idx ON consulta_anexos (consulta_id, criado_em);
...
SELECT
  c.id, c.codigo_atendimento, c.entidade_contratante_id, c.unidade_ubt_id,
  c.paciente_id, c.profissional_id, c.especialidade_id, c.agenda_consulta_id,
  c.fila_espera_id, c.tipo, c.status, c.triagem_resumo, c.notas_clinicas,
  c.iniciada_em, c.finalizada_em, c.duracao_minutos, c.cancelada_em,
  c.criado_em, c.atualizado_em,
  p.nome AS paciente_nome, p.cpf AS paciente_cpf, p.sexo AS paciente_sexo,
  p.data_nascimento AS paciente_data_nascimento, p.endereco AS paciente_endereco,
  p.foto_url AS paciente_foto_url,
  pr.nome AS profissional_nome, pr.especialidade AS profissional_especialidade_texto,
  e.nome AS especialidade_nome, ub.nome AS unidade_nome,
  ub.ra_chave AS unidade_regiao_chave, ub.ra_rotulo AS unidade_regiao_rotulo
FROM consultas c
