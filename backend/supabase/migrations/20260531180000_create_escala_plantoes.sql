-- Etapa 3.1 — Motor operacional de escala/plantões
-- Domínio cross-tenant (admin Telefarmed). Portal profissional: escopo fixo profissional_id do JWT.
-- Regra de ouro: nunca autorizar por profissional_id / entidade / UBT vindos do body em rotas de portal.

CREATE TYPE escala_modalidade AS ENUM (
  'tele',
  'hibrido',
  'presencial_ubt'
);

CREATE TYPE escala_modo_atribuicao AS ENUM (
  'assigned',
  'open'
);

CREATE TYPE escala_modo_programacao AS ENUM (
  'single',
  'fechada'
);

CREATE TYPE escala_escopo_selecao AS ENUM (
  'all',
  'selected'
);

CREATE TYPE escala_escopo_ubt AS ENUM (
  'all',
  'selected',
  'tele_only'
);

CREATE TYPE status_escala_programacao AS ENUM (
  'rascunho',
  'publicada',
  'cancelada'
);

CREATE TYPE status_escala_slot AS ENUM (
  'rascunho',
  'publicada',
  'cancelada',
  'encerrada'
...
CREATE TYPE status_escala_inscricao AS ENUM (
  'pendente',
  'aceita',
  'rejeitada',
...
CREATE TYPE status_escala_plantao AS ENUM (
  'confirmado',
  'realizado',
  'cancelado',
  'falta_profissional'
...
CREATE TABLE escala_programacoes (
...
COMMENT ON TABLE escala_programacoes IS
  'Lotes de programação de escala (período, modalidade, escopos). Cross-tenant — admin Telefarmed. '
  'Rotas portal: escopo derivado do JWT; filtros admin opcionais via query params.';

COMMENT ON COLUMN escala_programacoes.templates IS
  'Templates de blocos recorrentes (weekdays, horários, especialidade) usados na geração de escala_slots.';

CREATE INDEX escala_programacoes_status_idx
  ON escala_programacoes (status, data_inicio DESC);

CREATE INDEX escala_programacoes_periodo_idx
  ON escala_programacoes (data_inicio, data_fim);

CREATE INDEX escala_programacoes_criado_por_idx
...
COMMENT ON TABLE escala_slots IS
  'Instâncias concretas de plantão (data/hora/especialidade/vagas). '
  'Admin: cross-tenant. Profissional: marketplace filtra slots publicados em modo open; '
  'inscrições/plantões próprios filtrados por profissional_id do JWT.';

COMMENT ON COLUMN escala_slots.fila_reserva IS
  'UUIDs de usuarios_profissionais em ordem de reserva (modo assigned).';

CREATE INDEX escala_slots_data_idx
  ON escala_slots (data, hora_inicio);

CREATE INDEX escala_slots_especialidade_idx
  ON escala_slots (especialidade_id);

CREATE INDEX escala_slots_status_publicada_idx
  ON escala_slots (status, data, hora_inicio)
  WHERE status = 'publicada';

CREATE INDEX escala_slots_programacao_idx
  ON escala_slots (programacao_id)
  WHERE programacao_id IS NOT NULL;

CREATE INDEX escala_slots_lote_idx
  ON escala_slots (lote_id)
  WHERE lote_id IS NOT NULL;

CREATE INDEX escala_slots_titular_idx
...
CREATE TABLE escala_inscricoes_profissional (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
COMMENT ON TABLE escala_inscricoes_profissional IS
  'Candidatura do profissional a um slot aberto. '
  'Portal profissional: CRUD restrito ao profissional_id do JWT (applyProfissionalScope).';

CREATE INDEX escala_inscricoes_profissional_idx
  ON escala_inscricoes_profissional (profissional_id, status, inscrito_em DESC);

CREATE INDEX escala_inscricoes_slot_idx
...
CREATE TABLE escala_plantoes_confirmados (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  slot_id UUID NOT NULL REFERENCES escala_slots(id) ON DELETE CASCADE,
...
COMMENT ON TABLE escala_plantoes_confirmados IS
  'Plantão confirmado (slot + profissional). Titular assigned ou aceite de inscrição. '
  'Profissional: leitura filtrada por profissional_id do JWT.';

CREATE INDEX escala_plantoes_profissional_idx
  ON escala_plantoes_confirmados (profissional_id, status, confirmado_em DESC);

CREATE INDEX escala_plantoes_slot_idx
  ON escala_plantoes_confirmados (slot_id, status);

CREATE INDEX escala_plantoes_ativos_idx
...
-- Views de listagem (backend service_role)
...
COMMENT ON VIEW vw_admin_escala_slots_listagem IS
  'Listagem admin de slots com KPIs de preenchimento. Cross-tenant.';
...
COMMENT ON VIEW vw_profissional_escala_slots_disponiveis IS
  'Marketplace de plantões abertos para o portal profissional. '
  'Inscrições/plantões do profissional: filtrar escala_inscricoes_profissional e '
  'escala_plantoes_confirmados com applyProfissionalScope(profissional_id).';
