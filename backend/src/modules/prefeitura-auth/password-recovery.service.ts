...
async function rejectInvalidRecoveryCode(): Promise<never> {
  throw new PrefeituraPasswordRecoveryError('Código inválido ou expirado.', 'INVALID_CODE')
}
...
async function findPrefeituraUserByCpf(cpf: string): Promise<{
  id: string
  email: string | null
  status: string
} | null> {
...
async function invalidatePendingRecoveries(userId: string): Promise<void> {
  const { error } = await supabaseAdmin
    ...
    .delete()
    .eq('usuario_prefeitura_id', userId)
...
export async function requestPrefeituraPasswordRecovery(cpfInput: string): Promise<{
  resetToken: string
  sentTo: string
  ...
  try {
    ...
    throw new PrefeituraPasswordRecoveryError('Informe um CPF válido.', 'INVALID_CPF')
  }

  const user = await findPrefeituraUserByCpf(cpf)
  if (!user) {
  ...
  if (user.status !== 'ativo') {
    throw new PrefeituraPasswordRecoveryError(
      'Este acesso da prefeitura está inativo. Entre em contato com o suporte.',
      'USER_INACTIVE',
      403,
    )
  }
  ...
  if (!user.email) {
    throw new PrefeituraPasswordRecoveryError(
      'Não há e-mail cadastrado para este usuário da prefeitura. Solicite atualização ao administrador municipal.',
      'EMAIL_NOT_CONFIGURED',
      400,
    )
  }
  ...
  const { error } = await supabaseAdmin.from('prefeitura_recuperacao_senha').insert({
    usuario_prefeitura_id: user.id,
  ...
  try {
    ...
    console.error('[prefeitura-password-recovery] Falha ao enviar e-mail:', mailError)
    throw new PrefeituraPasswordRecoveryError(
...
async function findOpenRecoveryByResetToken(resetToken: string): Promise<RecoveryRow | null> {
  ...
  const { data, error } = await supabaseAdmin
    .from('prefeitura_recuperacao_senha')
    .select(
      'id, usuario_prefeitura_id, hash_codigo, expira_em, verificado_em, concluido_em, tentativas_codigo',
    )
  ...
  return {
    id: String(data.id),
    usuario_prefeitura_id: String(data.usuario_prefeitura_id),
...
export async function verifyPrefeituraPasswordRecoveryCode(input: {
  resetToken: string
  code: string
  ...
  try {
    ...
    if (new Date(recovery.expira_em).getTime() <= Date.now()) {
      await supabaseAdmin.from('prefeitura_recuperacao_senha').delete().eq('id', recovery.id)
...
export async function completePrefeituraPasswordRecovery(input: {
  verificationToken: string
  password: string
}): Promise<void> {
  ...
  const { data: userRow, error: userError } = await supabaseAdmin
    .from('usuarios_prefeitura')
    .select('id, status')
    .eq('id', recovery.usuario_prefeitura_id)
    .maybeSingle()
  ...
  const { error: revokeSessionsError } = await supabaseAdmin
    ...
    .eq('usuario_id', recovery.usuario_prefeitura_id)
    .is('revogado_em', null)
...
export function mapPrefeituraPasswordRecoveryError(error: unknown): {
  ...
  if (error instanceof PrefeituraPasswordRecoveryError) {
    return {
