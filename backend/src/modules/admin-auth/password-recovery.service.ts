...
async function rejectInvalidRecoveryCode(): Promise<never> {
  throw new AdminPasswordRecoveryError('Código inválido ou expirado.', 'INVALID_CODE')
}
...
async function findAdminUserByCpf(cpf: string): Promise<{
  id: string
  email: string | null
  status: string
} | null> {
...
export async function requestAdminPasswordRecovery(cpfInput: string): Promise<{
  resetToken: string
  sentTo: string
  ...
  try {
    ...
    throw new AdminPasswordRecoveryError('Informe um CPF válido.', 'INVALID_CPF')
  }

  const user = await findAdminUserByCpf(cpf)
  if (!user) {
  ...
  if (user.status !== 'ativo') {
    throw new AdminPasswordRecoveryError(
      'Este acesso administrativo está inativo. Entre em contato com o suporte.',
      'USER_INACTIVE',
      403,
    )
  }
  ...
  if (!user.email) {
    throw new AdminPasswordRecoveryError(
      'Não há e-mail cadastrado para este usuário administrativo. Solicite atualização ao administrador master.',
      'EMAIL_NOT_CONFIGURED',
      400,
    )
  }
  ...
  const { error } = await supabaseAdmin.from('admin_recuperacao_senha').insert({
  ...
  try {
    ...
    console.error('[admin-password-recovery] Falha ao enviar e-mail:', mailError)
    throw new AdminPasswordRecoveryError(
...
async function findOpenRecoveryByResetToken(resetToken: string): Promise<RecoveryRow | null> {
  ...
  const { data, error } = await supabaseAdmin
    .from('admin_recuperacao_senha')
    .select(
      'id, usuario_admin_id, hash_codigo, expira_em, verificado_em, concluido_em, tentativas_codigo',
    )
  ...
  return {
    id: String(data.id),
    usuario_admin_id: String(data.usuario_admin_id),
...
export async function verifyAdminPasswordRecoveryCode(input: {
  resetToken: string
  code: string
  ...
  try {
    ...
    if (new Date(recovery.expira_em).getTime() <= Date.now()) {
      await supabaseAdmin.from('admin_recuperacao_senha').delete().eq('id', recovery.id)
...
export async function completeAdminPasswordRecovery(input: {
  verificationToken: string
  password: string
}): Promise<void> {
  ...
  const { data: userRow, error: userError } = await supabaseAdmin
    .from('usuarios_admin')
    .select('id, status')
    .eq('id', recovery.usuario_admin_id)
    .maybeSingle()
  ...
  const { error: revokeSessionsError } = await supabaseAdmin
    ...
    .eq('usuario_id', recovery.usuario_admin_id)
    .is('revogado_em', null)
...
export function mapAdminPasswordRecoveryError(error: unknown): {
  ...
  if (error instanceof AdminPasswordRecoveryError) {
    return {
