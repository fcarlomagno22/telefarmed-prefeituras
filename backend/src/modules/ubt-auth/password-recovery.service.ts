...
import { revokeUbtLgpdUnlocks } from './lgpd.service.js'
...
async function rejectInvalidRecoveryCode(): Promise<never> {
  throw new UbtPasswordRecoveryError('Código inválido ou expirado.', 'INVALID_CODE')
}
...
async function findUbtUserByCpf(cpf: string): Promise<{
  id: string
  email: string | null
  status: string
  unidadeUbtId: string
...
export async function requestUbtPasswordRecovery(cpfInput: string): Promise<{
  resetToken: string
  sentTo: string
  ...
  try {
    ...
    throw new UbtPasswordRecoveryError('Informe um CPF válido.', 'INVALID_CPF')
  }

  const user = await findUbtUserByCpf(cpf)
  if (!user) {
  ...
  if (user.status !== 'ativo') {
    throw new UbtPasswordRecoveryError(
      'Este acesso UBT está inativo. Entre em contato com o suporte.',
      'USER_INACTIVE',
      403,
    )
  }
  ...
  if (!user.email) {
    throw new UbtPasswordRecoveryError(
      'Não há e-mail cadastrado para este operador UBT. Solicite atualização ao administrador.',
      'EMAIL_NOT_CONFIGURED',
      400,
    )
  }
  ...
  const { error } = await supabaseAdmin.from('ubt_recuperacao_senha').insert({
    usuario_ubt_id: user.id,
  ...
  try {
    ...
    console.error('[ubt-password-recovery] Falha ao enviar e-mail:', mailError)
    throw new UbtPasswordRecoveryError(
...
async function findOpenRecoveryByResetToken(resetToken: string): Promise<RecoveryRow | null> {
  ...
  const { data, error } = await supabaseAdmin
    .from('ubt_recuperacao_senha')
    .select(
      'id, usuario_ubt_id, hash_codigo, expira_em, verificado_em, concluido_em, tentativas_codigo',
    )
  ...
  return {
    id: String(data.id),
    usuario_ubt_id: String(data.usuario_ubt_id),
...
export async function verifyUbtPasswordRecoveryCode(input: {
  resetToken: string
  code: string
  ...
  try {
    ...
    if (new Date(recovery.expira_em).getTime() <= Date.now()) {
      await supabaseAdmin.from('ubt_recuperacao_senha').delete().eq('id', recovery.id)
...
export async function completeUbtPasswordRecovery(input: {
  verificationToken: string
  password: string
}): Promise<void> {
  ...
  const { error: revokeSessionsError } = await supabaseAdmin
    ...
    .eq('usuario_id', recovery.usuario_ubt_id)
    .is('revogado_em', null)
...
export function mapUbtPasswordRecoveryError(error: unknown): {
  statusCode: number
  body: { error: string; code?: string }
} {
  if (error instanceof UbtPasswordRecoveryError) {
    return {
