...
const canView = requirePrefeituraPagePermission('auditoria', 'visualizar')

export async function registerPrefeituraAuditoriaRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get('/', { preHandler: canView }, async (request, reply) => {
    ...
    try {
      const scope = await resolveAuditoriaScopeFilter('prefeitura', {
