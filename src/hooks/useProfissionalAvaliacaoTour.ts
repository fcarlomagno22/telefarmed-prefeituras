...
import {
  profissionalAvaliacaoTourSteps,
...
import {
  isProfissionalAvaliacaoTourInvitePending,
  markProfissionalAvaliacaoTourCompleted,
  markProfissionalAvaliacaoTourInviteHandled,
} from '../utils/profissional/profissionalAvaliacaoTourStorage'

type TourAdvanceSource = 'next' | 'target-click'

type UseProfissionalAvaliacaoTourOptions = {
  forceStart?: boolean
  onBeforeAdvance?: (step: ProfissionalAvaliacaoTourStep, source: TourAdvanceSource) => void
  onStepActive?: (step: ProfissionalAvaliacaoTourStep, stepIndex: number) => void
}
...
function resolveStepIndex(startIndex: number, direction: 1 | -1): number {
  ...
  const total = profissionalAvaliacaoTourSteps.length

  while (index >= 0 && index < total) {
    const step = profissionalAvaliacaoTourSteps[index]
...
const SLOW_TARGET_STEP_IDS = new Set(['tab-criticos', 'critical-review', 'review-card'])

export function useProfissionalAvaliacaoTour({
  forceStart = false,
  onBeforeAdvance,
  onStepActive,
}: UseProfissionalAvaliacaoTourOptions = {}) {
  ...
  const step: ProfissionalAvaliacaoTourStep = profissionalAvaliacaoTourSteps[stepIndex]
  const isLastStep = stepIndex >= profissionalAvaliacaoTourSteps.length - 1
  const totalSteps = profissionalAvaliacaoTourSteps.length
  ...
  const isInvitePending = useCallback(() => isProfissionalAvaliacaoTourInvitePending(), [])

  const { inviteOpen, acceptInvite, dismissInvite } = useProfissionalTourInviteGate({
    isInvitePending,
    markInviteHandled: markProfissionalAvaliacaoTourInviteHandled,
  ...
  const advanceToNextStep = useCallback(() => {
    setStepIndex((current) => {
      if (current >= profissionalAvaliacaoTourSteps.length - 1) {
        finishTour()
        return current
      }
