...
type UseProfissionalEscalaTourOptions = {
  forceStart?: boolean
  onBeforeAdvance?: (step: ProfissionalEscalaTourStep, source: TourAdvanceSource) => void
  onStepActive?: (step: ProfissionalEscalaTourStep, stepIndex: number) => void
}
...
function resolveStepIndex(startIndex: number, direction: 1 | -1): number {
  ...
  const total = profissionalEscalaTourSteps.length

  while (index >= 0 && index < total) {
    const step = profissionalEscalaTourSteps[index]
...
const SLOW_TARGET_STEP_IDS = new Set(['city-tooltip', 'claim-btn', 'claim-modal'])

export function useProfissionalEscalaTour({
  forceStart = false,
  onBeforeAdvance,
  onStepActive,
}: UseProfissionalEscalaTourOptions = {}) {
  ...
  const step: ProfissionalEscalaTourStep = profissionalEscalaTourSteps[stepIndex]
  const isLastStep = stepIndex >= profissionalEscalaTourSteps.length - 1
  const totalSteps = profissionalEscalaTourSteps.length
  ...
  const advanceToNextStep = useCallback(() => {
    setStepIndex((current) => {
      if (current >= profissionalEscalaTourSteps.length - 1) {
        finishTour()
        return current
      }
