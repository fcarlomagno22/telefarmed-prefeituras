...
type ExportContext = { report: CadastrosIncompletosReportApi; generatedAtLabel: string
...
function buildChart(points: Array<{ label: string; value: number }>, mode: 'daily' | 'monthly') {
  ...
  const bars = points
    ...
    .join('')
  return `<div class="chart-wrap"><div class="chart-bars">${bars}</div><p class="chart-caption">Evolução ${mode === 'monthly' ? 'mensal' : 'diária'} de cadastros incompletos na rede municipal.</p></div>`
}

export function buildCadastrosIncompletosReportHtml({ report, generatedAtLabel, branding: brandingInput }: ExportContext) {
  ...
  const sampleRows = report.samples
    .map(
      (row) =>
        `<tr><td class="unit">${escapeHtml(row.name)}</td><td class="metric">${escapeHtml(row.unitName)}</td><td class="metric">${escapeHtml(row.missingFields.join(', '))}</td><td class="metric">${row.missingCount}</td></tr>`,
    )
    .join('')
  const kpiCards = report.summary.kpis
    ...
    .join('')
  return applyEntidadeCopyToExportText(`<!DOCTYPE html><html lang="pt-BR"><head><meta charset="UTF-8" /><title>${escapeHtml(report.title)}</title><style>${buildStyles(branding)}</style></head><body><main><div class="brand-bar"></div><header class="header"><div><p class="eyebrow">Relatório operacional</p><h1>${escapeHtml(report.title)}</h1><p class="subtitle">${escapeHtml(report.description)}</p><div class="meta"><p><strong>${escapeHtml(report.entidadeRazaoSocial)}</strong> · ${escapeExportHtml(branding.brandName)}</p><p>Período: <strong>${escapeHtml(report.periodLabel)}</strong></p></div></div><img src="${escapeHtml(logoUrl)}" alt="${escapeExportHtml(branding.brandName)}" /></header><section><div class="summary-grid"><article class="card"><p class="card-label">Cadastros incompletos</p><p class="card-value">${formatNumber(report.summary.incompleteCount)}</p></article><article class="card"><p class="card-label">Taxa de incompletude</p><p class="card-value">${formatPercent(report.summary.incompleteRatePercent)}%</p></article><article class="card"><p class="card-label">Pacientes totais</p><p class="card-value">${formatNumber(report.summary.totalPatients)}</p></article><article class="card"><p class="card-label">Unidades impactadas</p><p class="card-value">${report.summary.unitsCount}</p></article></div></section><section><h2>Destaques do período</h2><div class="highlight-grid">${highlightCards}</div></section><section><h2>Campos com maior pendência</h2><table><thead><tr><th class="unit">Campo</th><th class="metric">Ocorrências</th><th class="metric">Participação</th></tr></thead><tbody>${fieldRows}</tbody></table></section><section><h2>Unidades com cadastros pendentes</h2><table><thead><tr><th class="unit">Unidade</th><th class="metric">Região</th><th class="metric">Pendências</th><th class="metric">Participação</th></tr></thead><tbody>${unitRows}</tbody></table></section><section><h2>Amostra de pacientes com pendência</h2><table><thead><tr><th class="unit">Paciente</th><th class="metric">Unidade</th><th class="metric">Campos faltantes</th><th class="metric">Qtde</th></tr></thead><tbody>${sampleRows}</tbody></table></section><section><h2>Evolução das pendências</h2>${buildChart(report.evolution.incompletePoints, report.evolution.mode)}</section><section><h2>Indicadores do período</h2><div class="kpi-grid">${kpiCards}</div></section><footer class="footer"><p>Relatório gerado em <strong>${escapeHtml(generatedAtLabel)}</strong> por <strong>${escapeHtml(report.generatedBy)}</strong></p><p>${escapeHtml(report.description)}</p></footer></main></body></html>`)
}
...
export async function exportPrefeituraCadastrosIncompletosReportPdf(context: ExportContext) {
  const filename = pdfFilenameFromLabel('cadastros-incompletos', context.report.periodLabel)
  const html = buildCadastrosIncompletosReportHtml(context)
