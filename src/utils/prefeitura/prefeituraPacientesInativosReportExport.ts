...
type ExportContext = { report: PacientesInativosReportApi; generatedAtLabel: string
...
function buildStyles(branding: EntidadeExportBranding = resolveEntidadeExportBranding()) {
  return buildEntidadeExportBaseStyles(branding.corPrimaria) + `
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .chart-bar { width: 100%; max-width: 32px; border-radius: 8px 8px 0 0; background: linear-gradient(to top, #ea580c, #fb923c); }
...
function buildChart(points: Array<{ label: string; value: number }>, mode: 'daily' | 'monthly') {
  ...
  const bars = points
    ...
    .join('')
  return `<div class="chart-wrap"><div class="chart-bars">${bars}</div><p class="chart-caption">Evolução ${mode === 'monthly' ? 'mensal' : 'diária'} da base inativa na rede municipal.</p></div>`
}

export function buildPacientesInativosReportHtml({ report, generatedAtLabel, branding: brandingInput }: ExportContext) {
  ...
  const kpiCards = report.summary.kpis
    ...
    .join('')
  return applyEntidadeCopyToExportText(`<!DOCTYPE html><html lang="pt-BR"><head><meta charset="UTF-8" /><title>${escapeHtml(report.title)}</title><style>${buildStyles(branding)}</style></head><body><main><div class="brand-bar"></div><header class="header"><div><p class="eyebrow">Relatório operacional</p><h1>${escapeHtml(report.title)}</h1><p class="subtitle">${escapeHtml(report.description)}</p><div class="meta"><p><strong>${escapeHtml(report.entidadeRazaoSocial)}</strong> · ${escapeExportHtml(branding.brandName)}</p><p>Período: <strong>${escapeHtml(report.periodLabel)}</strong></p></div></div><img src="${escapeHtml(logoUrl)}" alt="${escapeExportHtml(branding.brandName)}" /></header><section><div class="summary-grid"><article class="card"><p class="card-label">Pacientes inativos</p><p class="card-value">${formatNumber(report.summary.inactiveCount)}</p></article><article class="card"><p class="card-label">Taxa de inatividade</p><p class="card-value">${formatPercent(report.summary.inactiveRatePercent)}%</p></article><article class="card"><p class="card-label">Nunca consultaram</p><p class="card-value">${formatNumber(report.summary.neverConsultedCount)}</p></article><article class="card"><p class="card-label">Unidades impactadas</p><p class="card-value">${report.summary.unitsCount}</p></article></div></section><section><h2>Destaques do período</h2><div class="highlight-grid">${highlightCards}</div></section><section><h2>Faixas de inatividade</h2><table><thead><tr><th class="unit">Faixa</th><th class="metric">Pacientes</th><th class="metric">Participação</th></tr></thead><tbody>${bandRows}</tbody></table></section><section><h2>Unidades com pacientes inativos</h2><table><thead><tr><th class="unit">Unidade</th><th class="metric">Região</th><th class="metric">Pacientes</th><th class="metric">Participação</th></tr></thead><tbody>${unitRows}</tbody></table></section><section><h2>Evolução da inatividade</h2>${buildChart(report.evolution.inactivePoints, report.evolution.mode)}</section><section><h2>Indicadores do período</h2><div class="kpi-grid">${kpiCards}</div></section><footer class="footer"><p>Relatório gerado em <strong>${escapeHtml(generatedAtLabel)}</strong> por <strong>${escapeHtml(report.generatedBy)}</strong></p><p>${escapeHtml(report.description)}</p></footer></main></body></html>`)
}
...
export async function exportPrefeituraPacientesInativosReportPdf(context: ExportContext) {
  const filename = pdfFilenameFromLabel('pacientes-inativos', context.report.periodLabel)
  const html = buildPacientesInativosReportHtml(context)
