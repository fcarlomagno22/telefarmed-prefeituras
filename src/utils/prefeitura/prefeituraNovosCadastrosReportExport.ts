...
function buildStyles() {
  return `
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .chart-bar { width: 100%; max-width: 32px; border-radius: 8px 8px 0 0; background: linear-gradient(to top, #4f46e5, #818cf8); }
...
export async function exportPrefeituraNovosCadastrosReportPdf(context: ExportContext) {
  const filename = pdfFilenameFromLabel('novos-cadastros', context.report.periodLabel)
  const html = buildNovosCadastrosReportHtml(context)
