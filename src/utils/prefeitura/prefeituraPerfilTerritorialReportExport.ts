...
function buildStyles(branding: EntidadeExportBranding = resolveEntidadeExportBranding()) {
  return buildEntidadeExportBaseStyles(branding.corPrimaria) + `
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .chart-bar { width: 100%; max-width: 32px; border-radius: 8px 8px 0 0; background: linear-gradient(to top, #059669, #34d399); }
...
function buildChart(points: Array<{ label: string; value: number }>, mode: 'daily' | 'monthly') {
  ...
  const bars = points
    ...
    .join('')
  return `<div class="chart-wrap"><div class="chart-bars">${bars}</div><p class="chart-caption">Evolução ${mode === 'monthly' ? 'mensal' : 'diária'} da base cadastral na rede municipal.</p></div>`
}

export function buildPerfilTerritorialReportHtml({ report, generatedAtLabel, branding: brandingInput }: ExportContext) {
  ...
  const highlightCards = report.highlights
    ...
    .join('')
  const neighborhoodRows = report.neighborhoods
    .map(
      (row) =>
        `<tr><td class="unit">${escapeHtml(row.label)}</td><td class="metric">${formatNumber(row.patientsCount)}</td><td class="metric">${formatPercent(row.sharePercent)}%</td></tr>`,
    )
    .join('')
  ...
  const kpiCards = report.summary.kpis
    ...
    .join('')
  const html = `<!DOCTYPE html><html lang="pt-BR"><head><meta charset="UTF-8" /><title>${escapeHtml(report.title)}</title><style>${buildStyles(branding)}</style></head><body><main><div class="brand-bar"></div><header class="header"><div><p class="eyebrow">Relatório operacional</p><h1>${escapeHtml(report.title)}</h1><p class="subtitle">${escapeHtml(report.description)}</p><div class="meta"><p><strong>${escapeHtml(report.entidadeRazaoSocial)}</strong> · ${escapeHtml(branding.brandName)}</p><p>Período: <strong>${escapeHtml(report.periodLabel)}</strong></p></div></div><img src="${escapeHtml(logoUrl)}" alt="${escapeHtml(branding.brandName)}" /></header><section><div class="summary-grid"><article class="card"><p class="card-label">Base cadastral</p><p class="card-value">${formatNumber(report.summary.totalPatients)}</p></article><article class="card"><p class="card-label">Bairros mapeados</p><p class="card-value">${formatNumber(report.summary.neighborhoodsCount)}</p></article><article class="card"><p class="card-label">Cobertura territorial</p><p class="card-value">${formatPercent(report.summary.mappedPatientsPercent)}%</p></article><article class="card"><p class="card-label">Novos no período</p><p class="card-value">${formatNumber(report.summary.newInPeriod)}</p></article></div></section><section><h2>Destaques do período</h2><div class="highlight-grid">${highlightCards}</div></section><section><h2>Distribuição por bairro</h2><table><thead><tr><th class="unit">Bairro</th><th class="metric">Pacientes</th><th class="metric">Participação</th></tr></thead><tbody>${neighborhoodRows}</tbody></table></section><section><h2>Distribuição por região</h2><table><thead><tr><th class="unit">Região</th><th class="metric">Pacientes</th><th class="metric">Participação</th></tr></thead><tbody>${regionRows}</tbody></table></section><section><h2>Unidades e base cadastral</h2><table><thead><tr><th class="unit">Unidade</th><th class="metric">Região</th><th class="metric">Pacientes</th><th class="metric">Participação</th></tr></thead><tbody>${unitRows}</tbody></table></section><section><h2>Evolução da base</h2>${buildChart(report.evolution.patientsPoints, report.evolution.mode)}</section><section><h2>Indicadores do período</h2><div class="kpi-grid">${kpiCards}</div></section><footer class="footer"><p>Relatório gerado em <strong>${escapeHtml(generatedAtLabel)}</strong> por <strong>${escapeHtml(report.generatedBy)}</strong></p><p>${escapeHtml(report.description)}</p></footer></main></body></html>`
...
export async function exportPrefeituraPerfilTerritorialReportPdf(context: ExportContext) {
  const filename = pdfFilenameFromLabel('perfil-territorial', context.report.periodLabel)
  const html = buildPerfilTerritorialReportHtml(context)
