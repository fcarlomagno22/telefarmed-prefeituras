...
function buildReportStyles(branding: EntidadeExportBranding = resolveEntidadeExportBranding()) {
  return `
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .chart-bar-hourly { background: linear-gradient(to top, #0891b2, #22d3ee); }
    .chart-bar-weekday { background: linear-gradient(to top, #4f46e5, #818cf8); }
    ...
    .map(
      (item) => `
        <div class="chart-bar-col">
          <span class="chart-bar-value">${item.value > 0 ? formatNumber(item.value) : ''}</span>
          <div class="chart-bar ${barClass}" style="height:${Math.max(8, Math.round((item.value / max) * barAreaHeightPx))}px"></div>
...
function buildEvolutionChart(
    ...
    .map(
      (point) => `
        <div class="chart-bar-col">
          ...
          <div class="chart-bar chart-bar-volume" style="height:${Math.max(8, Math.round((point.value / max) * barAreaHeightPx))}px"></div>
  ...
  return `
    <div class="chart-wrap">
      <div class="chart-bars">${bars}</div>
      <p class="chart-caption">Evolução ${mode === 'monthly' ? 'mensal' : 'diária'} do volume total na rede municipal.</p>
    </div>
  `
}

export function buildHorariosPicoReportHtml({ report, generatedAtLabel }: ExportContext) {
  ...
  const unitRows = report.units
    .map(
      ...
      <tr>
        ...
        <td class="metric"><strong>${escapeHtml(unit.peakHour)}</strong></td>
        <td class="metric"><strong>${escapeHtml(unit.peakWeekday)}</strong></td>
        <td class="metric"><strong>${formatNumber(unit.peakVolume)}</strong></td>
        <td class="metric">${escapeHtml(unit.filaPeakHour)}</td>
        <td class="metric">${escapeHtml(unit.agendaPeakHour)}</td>
      </tr>
    `,
    )
    .join('')
  ...
  <main>
    ...
    <section>
      <div class="summary-grid">
        <article class="card">
          <p class="card-label">Pico horário</p>
          <p class="card-value">${escapeHtml(report.summary.peakHour)}</p>
          <p class="card-footer">${formatNumber(report.summary.peakHourVolume)} registros</p>
        </article>
        <article class="card">
          <p class="card-label">Pico semanal</p>
          <p class="card-value">${escapeHtml(report.summary.peakWeekday)}</p>
          <p class="card-footer">${formatNumber(report.summary.peakDayVolume)} registros</p>
        </article>
        <article class="card">
          <p class="card-label">Volume total</p>
          <p class="card-value">${formatNumber(report.summary.totalVolume)}</p>
          <p class="card-footer card-footer-positive">${signedPercent(report.summary.volumeDeltaPercent)} vs período anterior</p>
        </article>
        <article class="card">
          <p class="card-label">Unidades</p>
          <p class="card-value">${formatNumber(report.summary.unitsCount)}</p>
          <p class="card-footer">UBTs no recorte do relatório</p>
        </article>
      </div>
    </section>
    ${highlights}
    <section>
      ...
      ${buildDistributionChart(
        report.hourly.map((row) => ({ label: row.hour, value: row.totalCount })),
        'Volume combinado de agenda e fila por faixa horária.',
        'chart-bar-hourly',
      )}
    </section>
    <section>
      ...
      ${buildDistributionChart(
        report.weekday.map((row) => ({ label: row.label, value: row.totalCount })),
        'Volume combinado de agenda e fila por dia da semana.',
        'chart-bar-weekday',
      )}
    </section>
    <section>
      <h2>Evolução do volume total</h2>
      ${buildEvolutionChart(report.evolution.volumePoints, report.evolution.mode)}
    </section>
    <section>
      <h2>Picos por unidade</h2>
      <table>
        <thead>
          <tr>
            <th class="unit">Unidade</th>
            <th class="metric">Região</th>
            <th class="metric">Pico horário</th>
            <th class="metric">Pico semanal</th>
            <th class="metric">Volume no pico</th>
            <th class="metric">Pico fila</th>
            <th class="metric">Pico agenda</th>
          </tr>
        </thead>
    ...
    <footer class="footer">
      ...
      <p>Picos consolidados a partir dos registros de agenda e fila de espera nas UBTs da rede municipal.</p>
    </footer>
  </main>
</body>
</html>`)
...
export async function exportPrefeituraHorariosPicoReportPdf(context: ExportContext) {
  const filename = pdfFilenameFromLabel('horarios-pico', context.report.periodLabel)
  const html = buildHorariosPicoReportHtml(context)
