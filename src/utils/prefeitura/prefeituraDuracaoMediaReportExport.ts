...
type ExportContext = { report: DuracaoMediaReportApi; generatedAtLabel: string }
...
export function buildDuracaoMediaReportHtml({ report, generatedAtLabel }: ExportContext) {
  const summary = `<section><div class="summary-grid">
    <article class="card"><p class="card-label">Duração média</p><p class="card-value">${formatNumber(report.summary.avgDurationMinutes)} min</p><p class="card-footer card-footer-positive">${signedMinutes(report.summary.avgDurationDeltaMinutes)} vs período anterior</p></article>
    <article class="card"><p class="card-label">Mediana</p><p class="card-value">${formatNumber(report.summary.medianDurationMinutes)} min</p><p class="card-footer">Metade dos atendimentos abaixo deste tempo</p></article>
    <article class="card"><p class="card-label">Consultas analisadas</p><p class="card-value">${formatNumber(report.summary.completedCount)}</p></article>
    <article class="card"><p class="card-label">Outliers</p><p class="card-value">${formatNumber(report.summary.outliersCount)}</p></article>
  </div></section>`
  const specRows = report.specialties.map((r) => `<tr><td class="unit">${escapeHtml(r.name)}</td><td class="metric"><strong>${formatNumber(r.avgDurationMinutes)} min</strong></td><td class="metric">${formatNumber(r.medianDurationMinutes)} min</td><td class="metric">${formatNumber(r.completedCount)}</td><td class="metric">${formatPercent(r.sharePercent)}%</td><td class="metric">${signedMinutes(r.avgDurationDeltaMinutes)}</td><td class="metric">${r.isOutlier ? 'Sim' : '—'}</td></tr>`).join('')
  const profRows = report.professionals.map((r) => `<tr><td class="unit">${escapeHtml(r.name)}</td><td class="metric">${escapeHtml(r.especialidadeName)}</td><td class="metric"><strong>${formatNumber(r.avgDurationMinutes)} min</strong></td><td class="metric">${formatNumber(r.completedCount)}</td><td class="metric">${r.isOutlier ? 'Sim' : '—'}</td></tr>`).join('')
  const unitRows = report.units.map((r) => `<tr><td class="unit">${escapeHtml(r.name)}</td><td class="metric">${escapeHtml(r.region)}</td><td class="metric"><strong>${formatNumber(r.avgDurationMinutes)} min</strong></td><td class="metric">${formatNumber(r.medianDurationMinutes)} min</td><td class="metric">${formatNumber(r.completedCount)}</td><td class="metric ${r.durationVsNetworkMinutes <= 0 ? 'positive' : 'negative'}">${signedMinutes(r.durationVsNetworkMinutes)}</td></tr>`).join('')
  const body = summary + buildHighlights(report) + `<section><h2>Duração por especialidade</h2><table><thead><tr><th class="unit">Especialidade</th><th class="metric">Média</th><th class="metric">Mediana</th><th class="metric">Consultas</th><th class="metric">Participação</th><th class="metric">vs período ant.</th><th class="metric">Outlier</th></tr></thead><tbody>${specRows}</tbody></table></section><section><h2>Duração por profissional</h2><table><thead><tr><th class="unit">Profissional</th><th class="metric">Especialidade</th><th class="metric">Média</th><th class="metric">Consultas</th><th class="metric">Outlier</th></tr></thead><tbody>${profRows}</tbody></table></section><section><h2>Comparativo entre unidades</h2><table><thead><tr><th class="unit">Unidade</th><th class="metric">Região</th><th class="metric">Média</th><th class="metric">Mediana</th><th class="metric">Consultas</th><th class="metric">vs rede</th></tr></thead><tbody>${unitRows}</tbody></table></section><section><h2>Evolução da duração média</h2>${buildEvolutionChart(report.evolution.durationPoints, report.evolution.mode, 'duração média', 'chart-bar-primary', ' min')}</section><section><h2>Volume de consultas</h2>${buildEvolutionChart(report.evolution.volumePoints, report.evolution.mode, 'volume de consultas', 'chart-bar-volume')}</section><section><h2>Indicadores do período</h2>${buildKpiCards(report.summary.kpis)}</section>`
...
export async function exportPrefeituraDuracaoMediaReportPdf(context: ExportContext) {
  const filename = pdfFilenameFromLabel('duracao-media', context.report.periodLabel)
  const html = buildDuracaoMediaReportHtml(context)
