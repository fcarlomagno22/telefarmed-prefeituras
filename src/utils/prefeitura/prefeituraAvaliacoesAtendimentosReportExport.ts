...
type ExportContext = { report: AvaliacoesAtendimentosReportApi; generatedAtLabel: string }
...
export function buildAvaliacoesAtendimentosReportHtml({ report, generatedAtLabel }: ExportContext) {
  const summary = `<section><div class="summary-grid">
    <article class="card"><p class="card-label">Avaliações recebidas</p><p class="card-value">${formatNumber(report.summary.totalRatings)}</p><p class="card-footer card-footer-positive">${signedPercent(report.summary.ratingsDeltaPercent)} vs período anterior</p></article>
    <article class="card"><p class="card-label">Nota média geral</p><p class="card-value">${formatPercent(report.summary.avgRating, 1)}</p><p class="card-footer card-footer-positive">${signedPp(report.summary.avgRatingDeltaPp)} vs período anterior</p></article>
    <article class="card"><p class="card-label">Nota do profissional</p><p class="card-value">${formatPercent(report.summary.avgNotaProfissional, 1)}</p></article>
    <article class="card"><p class="card-label">Nota da teleconsulta</p><p class="card-value">${formatPercent(report.summary.avgNotaTeleconsulta, 1)}</p></article>
  </div></section>`
  const distRows = report.distribution.map((r) => `<tr><td class="unit">${r.rating} estrela(s)</td><td class="metric"><strong>${formatNumber(r.count)}</strong></td><td class="metric">${formatPercent(r.sharePercent)}%</td></tr>`).join('')
  const commentRows = report.comments.map((r) => `<tr><td class="unit">${escapeHtml(r.unitName)}</td><td class="metric">${escapeHtml(r.professionalName)}</td><td class="metric">${formatPercent(r.rating, 0)}</td><td>${escapeHtml(r.comment)}</td></tr>`).join('')
  const unitRows = report.units.map((r) => `<tr><td class="unit">${escapeHtml(r.name)}</td><td class="metric">${escapeHtml(r.region)}</td><td class="metric">${formatNumber(r.ratingsCount)}</td><td class="metric"><strong>${formatPercent(r.avgRating, 1)}</strong></td><td class="metric">${formatPercent(r.avgNotaProfissional, 1)}</td><td class="metric">${formatPercent(r.avgNotaTeleconsulta, 1)}</td><td class="metric ${r.avgRatingVsNetworkPp >= 0 ? 'positive' : 'negative'}">${signedPp(r.avgRatingVsNetworkPp)}</td></tr>`).join('')
  const body = summary + buildHighlights(report) + `<section><h2>Distribuição das notas</h2><table><thead><tr><th class="unit">Nota</th><th class="metric">Quantidade</th><th class="metric">Participação</th></tr></thead><tbody>${distRows}</tbody></table></section><section><h2>Comentários em destaque</h2><table><thead><tr><th class="unit">Unidade</th><th class="metric">Profissional</th><th class="metric">Nota</th><th>Comentário</th></tr></thead><tbody>${commentRows}</tbody></table></section><section><h2>Comparativo entre unidades</h2><table><thead><tr><th class="unit">Unidade</th><th class="metric">Região</th><th class="metric">Avaliações</th><th class="metric">Média</th><th class="metric">Profissional</th><th class="metric">Teleconsulta</th><th class="metric">vs rede</th></tr></thead><tbody>${unitRows}</tbody></table></section><section><h2>Volume de avaliações</h2>${buildEvolutionChart(report.evolution.ratingCountPoints, report.evolution.mode, 'avaliações recebidas', 'chart-bar-volume')}</section><section><h2>Evolução da nota média</h2>${buildEvolutionChart(report.evolution.avgRatingPoints, report.evolution.mode, 'nota média', 'chart-bar-success')}</section><section><h2>Indicadores do período</h2>${buildKpiCards(report.summary.kpis)}</section>`
...
export async function exportPrefeituraAvaliacoesAtendimentosReportPdf(context: ExportContext) {
  const filename = pdfFilenameFromLabel('avaliacoes-atendimentos', context.report.periodLabel)
  const html = buildAvaliacoesAtendimentosReportHtml(context)
