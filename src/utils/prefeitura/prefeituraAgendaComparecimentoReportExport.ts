...
function buildReportStyles() {
  return `
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .chart-bar-attendance { background: linear-gradient(to top, #0284c7, #38bdf8); }
    .chart-bar-noshow { background: linear-gradient(to top, #e11d48, #fb7185); }
    .chart-bar-rescheduled { background: linear-gradient(to top, #7c3aed, #a78bfa); }
...
function buildSummaryCards(report: AgendaComparecimentoReportApi) {
  const topUnit = report.units[0]
  const attendanceDeltaClass =
    report.summary.attendanceDeltaPp >= 0 ? 'card-footer-positive' : 'card-footer-negative'

  return `
    <div class="summary-grid">
      <article class="card">
        <p class="card-label">Agendamentos no período</p>
        <p class="card-value">${formatNumber(report.summary.scheduled)}</p>
        <p class="card-footer card-footer-positive">${signedPercent(report.summary.scheduledDeltaPercent)} vs período anterior</p>
      </article>
      <article class="card">
        <p class="card-label">Comparecimentos</p>
        <p class="card-value">${formatNumber(report.summary.attended)}</p>
        <p class="card-footer">Presença confirmada ou em andamento</p>
      </article>
      <article class="card">
        <p class="card-label">Taxa de comparecimento</p>
        <p class="card-value">${formatPercent(report.summary.attendanceRatePercent)}%</p>
        <p class="card-footer ${attendanceDeltaClass}">${signedPp(report.summary.attendanceDeltaPp)} vs período anterior</p>
      </article>
      <article class="card">
        <p class="card-label">Melhor desempenho</p>
...
function buildHighlights(report: AgendaComparecimentoReportApi) {
...
function buildUnitsTable(report: AgendaComparecimentoReportApi) {
  ...
  const rows = report.units
    .map((unit) => {
      const attendanceClass = unit.attendanceVsNetworkPp >= 0 ? 'positive' : 'negative'
      return `
        <tr>
          <td class="unit">${escapeHtml(unit.name)}</td>
          <td class="metric">${escapeHtml(unit.region)}</td>
          <td class="metric"><strong>${formatNumber(unit.scheduled)}</strong></td>
          <td class="metric">${formatNumber(unit.attended)}</td>
          <td class="metric">${formatNumber(unit.noShows)}</td>
          <td class="metric">${formatNumber(unit.pending)}</td>
          <td class="metric">${formatNumber(unit.rescheduled)}</td>
          <td class="metric"><strong>${formatPercent(unit.attendanceRatePercent)}%</strong></td>
          <td class="metric ${attendanceClass}">${signedPp(unit.attendanceVsNetworkPp)}</td>
        </tr>
      `
    })
    .join('')
    ...
    <table>
      <thead>
        <tr>
          ...
          <th class="metric">Compareceram</th>
          <th class="metric">Faltas</th>
          <th class="metric">Pendentes</th>
          <th class="metric">Remarcações</th>
          <th class="metric">Taxa comparec.</th>
...
function buildKpiCards(report: AgendaComparecimentoReportApi) {
...
export function buildAgendaComparecimentoReportHtml({ report, generatedAtLabel }: ExportContext) {
...
<body>
  <main>
    ...
    <section>
      <h2>Evolução da taxa de comparecimento</h2>
      ${buildEvolutionChart(
        report.evolution.attendancePoints,
        report.evolution.mode,
        'taxa de comparecimento',
        'chart-bar-attendance',
        '%',
      )}
    </section>

    <section>
      <h2>Evolução de faltas</h2>
      ${buildEvolutionChart(
        report.evolution.noShowPoints,
        report.evolution.mode,
        'faltas',
        'chart-bar-noshow',
      )}
    ...
    <section>
      <h2>Evolução de remarcações e cancelamentos</h2>
      ${buildEvolutionChart(
        report.evolution.rescheduledPoints,
        report.evolution.mode,
        'remarcações e cancelamentos',
        'chart-bar-rescheduled',
      )}
    </section>

    <section>
      <h2>Volume de agendamentos</h2>
      ${buildEvolutionChart(
    ...
    <footer class="footer">
      ...
      <p>Dados consolidados a partir das agendas das UBTs da rede municipal e dos status de comparecimento, falta, cancelamento ou remarcação.</p>
    </footer>
  </main>
</body>
</html>`
...
export async function exportPrefeituraAgendaComparecimentoReportPdf(context: ExportContext) {
  const filename = pdfFilenameFromLabel('agenda-vs-comparecimento', context.report.periodLabel)
  const html = buildAgendaComparecimentoReportHtml(context)
