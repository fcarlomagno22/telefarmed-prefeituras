...
function computeKpis(rows: Record<string, unknown>[]) {
  return [
    { key: 'rows', label: 'Registros', value: String(rows.length), suffix: 'no período' },
    { key: 'export', label: 'Exportação', value: 'Disponível', suffix: 'CSV' },
  ]
}

export function isPrefeituraRelatoriosApiError(error: unknown): error is PrefeituraRelatoriosApiError {
  return error instanceof PrefeituraRelatoriosApiError
}

export async function fetchPrefeituraRelatorioCategories(_accessToken: string) {
  void _accessToken
...
export async function fetchPrefeituraRelatorioMeta(_accessToken: string, categoryId: string) {
  void _accessToken
  const normalized = reportCategories.find((item) => item.id === categoryId)?.id
  if (!normalized) {
    throw new PrefeituraRelatoriosApiError('Categoria de relatório não encontrada.', 404, 'CATEGORY_NOT_FOUND')
  }
  return mockDelay(getMetaByCategory(normalized))
}

export async function fetchPrefeituraRelatorioData(
  _accessToken: string,
  categoryId: string,
  ...
  const paginated = filters.all
    ? { rows, page: 1, pageSize: rows.length || 1, total: rows.length, totalPages: 1 }
    : paginateRows(rows, filters.page ?? 1)
...
export async function downloadPrefeituraRelatorioCsv(
  accessToken: string,
  categoryId: string,
  filters: RelatoriosFiltersInput,
) {
  const data = await fetchPrefeituraRelatorioData(accessToken, categoryId, { ...filters, all: true })
