...
import {
  CRM_PARTNERS_PASSWORD_RECOVERY_CODE_LENGTH,
  type CrmPartnersPasswordRecoveryStepId,
} from '../../../config/crmPartnersPasswordRecovery'
import {
  crmPartnersCompletePasswordRecovery,
  crmPartnersRequestPasswordRecovery,
  crmPartnersVerifyPasswordRecoveryCode,
  CrmPartnersPasswordRecoveryError,
} from '../../../lib/services/crmPartners/passwordRecovery'
...
import { CrmPartnersPasswordRecoveryStepper } from './CrmPartnersPasswordRecoveryStepper'

const lottieInnerClass = 'h-full w-full shrink-0 [&_svg]:h-full [&_svg]:w-full'
const primaryButtonClass =
  'w-full rounded-xl bg-gradient-to-r from-orange-500 to-amber-500 py-3 text-sm font-semibold text-white transition hover:brightness-105 disabled:cursor-not-allowed disabled:opacity-50'
const inputClass =
  'w-full rounded-xl border border-gray-200 bg-white py-3 pl-11 pr-4 text-sm text-gray-800 outline-none transition placeholder:text-gray-400 focus:border-orange-500 focus:ring-2 focus:ring-orange-500/10'
...
type CrmPartnersPasswordRecoveryDrawerProps = {
  open: boolean
  closing: boolean
  onClose: () => void
...
export function CrmPartnersPasswordRecoveryDrawer({
  open,
  closing,
  onClose,
  onTransitionEnd,
}: CrmPartnersPasswordRecoveryDrawerProps) {
  ...
  return createPortal(
    ...
    >
      ...
      <aside
        role="dialog"
        aria-modal="true"
        aria-labelledby="crm-partners-password-recovery-title"
      ...
      >
        {isActive ? (
          <CrmPartnersPasswordRecoveryDrawerContent key={sessionKey} onClose={onClose} />
        ) : null}
      </aside>
    </div>,
    document.body,
...
type CrmPartnersPasswordRecoveryDrawerContentProps = {
  onClose: () => void
}

function CrmPartnersPasswordRecoveryDrawerContent({ onClose }: CrmPartnersPasswordRecoveryDrawerContentProps) {
  const [step, setStep] = useState<CrmPartnersPasswordRecoveryStepId>('cpf')
  ...
  async function handleCpfSubmit(event: FormEvent) {
    ...
    try {
      const result = await crmPartnersRequestPasswordRecovery(cpf)
      ...
      setSubmitError(
        error instanceof CrmPartnersPasswordRecoveryError
          ? error.message
  ...
  async function handleCodeSubmit(event: FormEvent) {
    ...
    if (code.length !== CRM_PARTNERS_PASSWORD_RECOVERY_CODE_LENGTH || !resetToken) return

    setIsSubmitting(true)
    try {
      const result = await crmPartnersVerifyPasswordRecoveryCode({ resetToken, code })
  ...
  async function handlePasswordSubmit(event: FormEvent) {
    ...
    setIsSubmitting(true)
    try {
      await crmPartnersCompletePasswordRecovery({ verificationToken, password })
      setStep('success')
    } catch (error) {
      setSubmitError(
        error instanceof CrmPartnersPasswordRecoveryError
  ...
  const stepSubtitle =
    step === 'cpf'
      ? 'Informe o CPF do parceiro comercial cadastrado.'
      : step === 'code'
        ? `Digite o código de ${CRM_PARTNERS_PASSWORD_RECOVERY_CODE_LENGTH} dígitos enviado para ${sentToEmail}.`
        : 'Defina uma nova senha para acessar o CRM Partners.'

  return (
    <div className="flex min-h-0 flex-1 flex-col">
      <header className="flex shrink-0 items-center justify-between px-6 pt-6">
        {showChrome ? (
          <h2 id="crm-partners-password-recovery-title" className="text-lg font-semibold text-gray-900">
            Recuperar senha
          </h2>
        ) : (
      ...
      {showStepper ? <CrmPartnersPasswordRecoveryStepper currentStep={step} /> : null}
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-6 py-6">
          ...
          {step === 'code' ? (
            <form className="space-y-5" onSubmit={handleCodeSubmit} noValidate>
              <PinInput
                id="crm-partners-recovery-code"
                label={`Código de ${CRM_PARTNERS_PASSWORD_RECOVERY_CODE_LENGTH} dígitos`}
                value={code}
                onChange={setCode}
                length={CRM_PARTNERS_PASSWORD_RECOVERY_CODE_LENGTH}
                visible={showCode}
              ...
              <button
                type="submit"
                disabled={isSubmitting || code.length !== CRM_PARTNERS_PASSWORD_RECOVERY_CODE_LENGTH}
                className={primaryButtonClass}
              >
              ...
              <div className="flex items-center justify-between pt-1">
                ...
                <button
                  ...
                  disabled={isSubmitting}
                  className="text-sm text-orange-600 transition hover:underline disabled:opacity-50"
                >
                  Reenviar código
                </button>
              </div>
