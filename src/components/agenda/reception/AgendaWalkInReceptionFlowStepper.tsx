...
import {
  resolveWalkInReceptionStepIndex,
  walkInReceptionFlowSteps,
  type AgendaWalkInReceptionStep,
} from './agendaWalkInReceptionTypes'

type AgendaWalkInReceptionFlowStepperProps = {
  step: AgendaWalkInReceptionStep
}

export function AgendaWalkInReceptionFlowStepper({
  step,
}: AgendaWalkInReceptionFlowStepperProps) {
  const activeIndex = resolveWalkInReceptionStepIndex(step)
  ...
  return (
    <nav
      aria-label="Progresso da recepção presencial"
    ...
    >
      ...
      <div className="flex gap-1.5">
        {walkInReceptionFlowSteps.map((flowStep, index) => {
