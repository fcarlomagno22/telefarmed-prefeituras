...
import {
  resolveScheduleStepIndex,
  scheduleFlowSteps,
  type ScheduleAppointmentStep,
} from './scheduleAppointmentTypes'

type ScheduleAppointmentFlowStepperProps = {
  step: ScheduleAppointmentStep
}

export function ScheduleAppointmentFlowStepper({ step }: ScheduleAppointmentFlowStepperProps) {
  const activeIndex = resolveScheduleStepIndex(step)
  const [fillRatio, setFillRatio] = useState(0)
  const fillRatioRef = useRef(0)

  const total = scheduleFlowSteps.length
  const allDone = step === 'success'
  const stepNumber = allDone ? total : activeIndex + 1
  const currentLabel = allDone
    ? 'Agendamento confirmado'
    : scheduleFlowSteps[activeIndex]?.label ?? ''
  ...
  return (
    ...
    >
      ...
      >
        {scheduleFlowSteps.map((flowStep, index) => {
