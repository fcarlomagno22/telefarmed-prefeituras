...
import {
  ubtCompletePasswordRecovery,
  ubtRequestPasswordRecovery,
  ubtVerifyPasswordRecoveryCode,
  UbtPasswordRecoveryError,
} from '../../../lib/services/ubt/passwordRecovery'
...
import { UbtPasswordRecoveryLottie } from './UbtPasswordRecoveryLottie'
import { UbtPasswordRecoveryLottieHero } from './UbtPasswordRecoveryLottieHero'
import { UbtPasswordRecoveryPinValidationLottie } from './UbtPasswordRecoveryPinValidationLottie'
import { UbtPasswordRecoverySuccessPanel } from './UbtPasswordRecoverySuccessPanel'
import { UbtPasswordRecoveryStepper } from './UbtPasswordRecoveryStepper'
...
type UbtPasswordRecoveryDrawerProps = {
  open: boolean
  closing: boolean
  onClose: () => void
...
export function UbtPasswordRecoveryDrawer({
  open,
  closing,
  onClose,
  onTransitionEnd,
}: UbtPasswordRecoveryDrawerProps) {
  ...
  return createPortal(
    ...
    >
      ...
      <aside
        role="dialog"
        aria-modal="true"
        aria-labelledby="ubt-password-recovery-title"
      ...
      >
        {isActive ? (
          <UbtPasswordRecoveryDrawerContent key={sessionKey} onClose={onClose} />
        ) : null}
      </aside>
    </div>,
    document.body,
...
type UbtPasswordRecoveryDrawerContentProps = {
  onClose: () => void
}
...
function UbtPasswordRecoveryDrawerContent({ onClose }: UbtPasswordRecoveryDrawerContentProps) {
  ...
  async function handleCpfSubmit(event: FormEvent) {
    ...
    try {
      const result = await ubtRequestPasswordRecovery(cpf)
      ...
      setSubmitError(
        error instanceof UbtPasswordRecoveryError
          ? error.message
  ...
  async function handleCodeSubmit(event: FormEvent) {
    ...
    if (code.length !== UBT_PASSWORD_RECOVERY_CODE_LENGTH || !resetToken) return

    setIsSubmitting(true)
    try {
      const result = await ubtVerifyPasswordRecoveryCode({ resetToken, code })
  ...
  async function handlePasswordSubmit(event: FormEvent) {
    ...
    setIsSubmitting(true)
    try {
      await ubtCompletePasswordRecovery({ verificationToken, password })
      setStep('success')
    } catch (error) {
      setSubmitError(
        error instanceof UbtPasswordRecoveryError
  ...
  const stepSubtitle =
    step === 'cpf'
      ? 'Informe o CPF do operador UBT cadastrado.'
      : step === 'code'
        ? `Digite o código de ${UBT_PASSWORD_RECOVERY_CODE_LENGTH} dígitos enviado para ${sentToEmail}.`
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      <header className="flex shrink-0 items-center justify-between px-6 pt-6">
        {showChrome ? (
          <h2 id="ubt-password-recovery-title" className="text-lg font-semibold text-gray-900">
            Recuperar senha
          </h2>
        ) : (
      ...
      {showStepper ? <UbtPasswordRecoveryStepper currentStep={step} /> : null}
      ...
      {step === 'success' ? (
        ...
        {step === 'code' ? (
          <form className="space-y-5" onSubmit={handleCodeSubmit} noValidate>
            <PinInput
              id="ubt-recovery-code"
              label={`Código de ${UBT_PASSWORD_RECOVERY_CODE_LENGTH} dígitos`}
              value={code}
              onChange={setCode}
              length={UBT_PASSWORD_RECOVERY_CODE_LENGTH}
              visible={showCode}
            ...
            <button
              type="submit"
              disabled={isSubmitting || code.length !== UBT_PASSWORD_RECOVERY_CODE_LENGTH}
              className={primaryButtonClass}
            >
