...
export type FonoaudiologoAtestadoSignedPayload = {
...
type FonoaudiologoAtestadoModalProps = {
  open: boolean
  onClose: () => void
  onSigned?: (payload: FonoaudiologoAtestadoSignedPayload) => void | Promise<void>
  patient: PsychologistPatientInfo
  professional: PsychologistProfessionalInfo
}

export function FonoaudiologoAtestadoModal({
  open,
  onClose,
  onSigned,
  patient,
  professional,
}: FonoaudiologoAtestadoModalProps) {
  ...
  async function handleSign() {
    if (tipo === 'afastamento' && !motivo.trim()) {
      setValidationHint('Informe o motivo do afastamento fonoaudiológico.')
      return
    }
    setSigning(true)
    setValidationHint(null)
    try {
      ...
      setValidationHint('Não foi possível emitir o atestado fonoaudiológico.')
    } finally {
      setSigning(false)
    }
  }
  ...
  return (
    <PsychologistDocumentModalShell
      ...
      subtitle="Comparecimento ou afastamento por motivos fonoaudiológicos"
      icon={FileText}
      accent="amber"
        ...
        {tipo === 'afastamento' ? (
          <>
            ...
            <TextAreaField
              ...
              required
              accent="amber"
              placeholder="Descreva a necessidade de afastamento com base na avaliação fonoaudiológica"
            />
          </>
        ) : null}
      </FormSection>
