...
export type FonoaudiologoParecerSignedPayload = {
  destinatario?: string
...
type FonoaudiologoParecerModalProps = {
  open: boolean
  onClose: () => void
  onSigned?: (payload: FonoaudiologoParecerSignedPayload) => void | Promise<void>
  patient: PsychologistPatientInfo
  professional: PsychologistProfessionalInfo
}

export function FonoaudiologoParecerModal({
  open,
  onClose,
  onSigned,
  patient,
  professional,
}: FonoaudiologoParecerModalProps) {
  ...
  async function handleSign() {
    ...
    try {
      ...
      setValidationHint('Não foi possível emitir o parecer fonoaudiológico.')
    } finally {
      setSigning(false)
    }
  }
