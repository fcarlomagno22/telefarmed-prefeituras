...
export type NutritionistParecerSignedPayload = {
  destinatario?: string
...
type NutritionistParecerModalProps = {
  open: boolean
  onClose: () => void
  onSigned?: (payload: NutritionistParecerSignedPayload) => void | Promise<void>
  patient: PsychologistPatientInfo
  professional: PsychologistProfessionalInfo
}

export function NutritionistParecerModal({
  open,
  onClose,
  onSigned,
  patient,
  professional,
}: NutritionistParecerModalProps) {
  ...
  async function handleSign() {
    ...
    try {
      ...
      setValidationHint('Não foi possível emitir o parecer nutricional.')
    } finally {
      setSigning(false)
    }
  }
