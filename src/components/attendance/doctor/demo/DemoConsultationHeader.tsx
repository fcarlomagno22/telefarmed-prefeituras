...
import type { DemoClinicalDocumentKind, DemoConsultationProfession } from '../../../../data/demoConsultationProfessions'
import { DemoIssueDocumentMegamenu } from './DemoIssueDocumentMegamenu'
import { DemoProfessionSelector } from './DemoProfessionSelector'

type DemoConsultationHeaderProps = {
  elapsed: string
  startedAtLabel: string
  headerTitle: string
  profession: DemoConsultationProfession
  onProfessionChange: (profession: DemoConsultationProfession) => void
  onFinishConsultation?: () => void
  onIssueDocument?: (kind: DemoClinicalDocumentKind) => void
  onViewPreviousConsultations?: () => void
}

export function DemoConsultationHeader({
  elapsed,
  startedAtLabel,
  headerTitle,
  profession,
...
}: DemoConsultationHeaderProps) {
  return (
    <header className="shrink-0 bg-[#f5f6f8] pb-3 pt-4">
      <div className="mb-3">
        <DemoProfessionSelector value={profession} onChange={onProfessionChange} />
      </div>

      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div className="min-w-0">
          <h1 className="text-xl font-bold tracking-tight text-gray-900 sm:text-[22px]">{headerTitle}</h1>
        ...
        <div className="flex flex-col items-stretch gap-2 sm:items-end">
          <div className="flex flex-wrap items-center justify-end gap-2">
            ...
            {onIssueDocument ? (
              <DemoIssueDocumentMegamenu profession={profession} onSelect={onIssueDocument} />
            ) : null}
            <button
              type="button"
