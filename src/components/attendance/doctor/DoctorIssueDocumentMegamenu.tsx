...
import {
  countDoctorClinicalDocuments,
  DOCTOR_CLINICAL_DOCUMENT_SECTIONS,
  type DoctorClinicalDocumentKind,
  type DoctorClinicalDocumentOption,
  type DoctorClinicalDocumentSection,
} from './doctorClinicalDocumentTypes'
...
type DoctorIssueDocumentMegamenuProps = {
  onSelect: (kind: DoctorClinicalDocumentKind) => void
}
...
function DocumentTile({
  ...
  item: DoctorClinicalDocumentOption
  onSelect: (kind: DoctorClinicalDocumentKind) => void
}) {
    ...
    >
      ...
      >
        ...
        {item.available ? (
          <span
            className={[
              'absolute -bottom-0.5 -right-0.5 h-2.5 w-2.5 rounded-full border-2 border-white bg-emerald-400 shadow-sm',
            ].join(' ')}
          />
        ) : null}
      </span>
...
function DocumentSection({
  ...
  section: DoctorClinicalDocumentSection
  onSelect: (kind: DoctorClinicalDocumentKind) => void
}) {
  ...
  return (
    <section className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-sm">
      <div className="flex items-center gap-3 border-b border-slate-100 px-4 py-3">
        <span
          className={[
            'h-9 w-1 shrink-0 rounded-full bg-gradient-to-b',
            section.accent.sectionBar,
          ].join(' ')}
        />
        <div className="min-w-0 flex-1">
...
export function DoctorIssueDocumentMegamenu({ onSelect }: DoctorIssueDocumentMegamenuProps) {
  const [open, setOpen] = useState(false)
  const [menuPosition, setMenuPosition] = useState<MenuPosition | null>(null)
  const containerRef = useRef<HTMLDivElement>(null)
  const triggerRef = useRef<HTMLButtonElement>(null)
  const menuRef = useRef<HTMLDivElement>(null)
  const documentCounts = countDoctorClinicalDocuments()
  ...
  function handleSelect(kind: DoctorClinicalDocumentKind) {
    setOpen(false)
    onSelect(kind)
  }
      ...
      <FloatingOverlayPortal>
        ...
        >
          <div className="relative overflow-hidden bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 px-5 py-4">
            ...
            <div className="pointer-events-none absolute -bottom-12 left-1/3 h-28 w-28 rounded-full bg-sky-400/15 blur-3xl" />
              ...
              <div className="flex min-w-0 items-start gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="mt-0.5 text-sm text-slate-300">
                    Escolha o tipo de documento para gerar nesta consulta
                  </p>
                </div>
              </div>
          ...
          <div className="max-h-[min(72vh,580px)] overflow-y-auto overscroll-y-contain bg-gradient-to-b from-slate-50 to-slate-100/80 p-4">
            <div className="space-y-3">
              {DOCTOR_CLINICAL_DOCUMENT_SECTIONS.map((section) => (
                <DocumentSection key={section.id} section={section} onSelect={handleSelect} />
              ))}
            </div>
          </div>

          <div className="flex items-center justify-between gap-3 border-t border-slate-200/80 bg-white px-5 py-3">
            <p className="text-xs text-slate-500">
              Documentos disponíveis abrem o formulário de emissão imediatamente.
            </p>
            <span className="hidden shrink-0 items-center gap-1.5 rounded-full bg-orange-50 px-2.5 py-1 text-[10px] font-semibold text-orange-700 ring-1 ring-orange-100 sm:inline-flex">
              <span className="h-1.5 w-1.5 rounded-full bg-emerald-500" />
              {documentCounts.available} prontos para uso
            </span>
          </div>
        </div>
      </FloatingOverlayPortal>
