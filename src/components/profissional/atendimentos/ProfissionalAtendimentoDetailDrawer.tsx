...
import { Clock3, FileText, Image, MapPin, Stethoscope, X } from 'lucide-react'
...
>(function ProfissionalAtendimentoDetailDrawer(
  ...
  useImperativeHandle(
    ...
    () => ({
      ...
      openReceivedPreview: (attachmentId?: string) => {
        if (!record) return
        const attachment =
          record.patientUploads.find((file) => file.id === attachmentId) ??
          record.patientUploads.find(
            (file) => file.id === PROFISSIONAL_ATENDIMENTOS_TOUR_PREVIEW_ATTACHMENT_ID,
          ) ??
          record.patientUploads[0]
  ...
  const patientProfile = useMemo(() => {
    ...
    return buildDoctorRecordPatientProfile({
      ...
      patientPhotoUrl: record.patientPhotoUrl,
      patientBirthDateIso: record.birthDateIso,
      patientCity: 'Telemedicina',
    })
  }, [record])

  const allRecordNotes = useMemo(() => {
    if (!record) return []
    const history = getDoctorRecordNotesForSpecialty(record.specialty)
    const merged = [...record.recordNotes, ...history]
  ...
  if (!isActive || !record || !patientProfile) return null

  const specialtyKey = resolveDoctorRecordSpecialtyKey(record.specialty)
  const specialtyLabel = DOCTOR_RECORD_SPECIALTY_LABELS[specialtyKey]
  const statusConfig = profissionalAtendimentosStatusConfig[record.status]
  ...
  return createPortal(
    <>
        ...
        >
          ...
          >
            <div className="flex items-start justify-between gap-3">
              <div className="flex min-w-0 flex-1 items-center gap-4">
                ...
                <div className="min-w-0">
                  <p className="text-xs font-semibold text-gray-500">{record.attendanceId}</p>
                  <h2
                  ...
                  <p className="mt-1 text-xs text-gray-500">
                    {record.dateTimeLabel} · {record.specialty}
                  </p>
                </div>
              </div>
              <button
            ...
            <dl className="mt-4 grid grid-cols-2 gap-x-4 gap-y-2 text-xs text-gray-600">
              <div className="flex items-center gap-1.5">
                ...
                <dd>
                  <span className="font-medium text-gray-500">Idade:</span> {record.age} anos
                </dd>
              </div>
              <div className="flex items-center gap-1.5">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <dd>{record.durationMinutes} min</dd>
              </div>
              <div className="flex items-center gap-1.5">
                <dd>
                  <span
          ...
          <div className="min-h-0 flex-1 overflow-y-auto overscroll-y-contain bg-[#f5f6f8] px-5 py-4 sm:px-6">
            <div className="flex flex-col gap-4">
              ...
              <ProfissionalAttendanceSentPanel
                documents={record.issuedDocuments}
                dataTour="atendimentos-drawer-sent"
              />
              <ProfissionalAttendanceReceivedPanel
                files={record.patientUploads}
                onPreview={setPreviewAttachment}
      ...
      <DoctorFullRecordModal
        ...
        doctorSpecialty={record.specialty}
