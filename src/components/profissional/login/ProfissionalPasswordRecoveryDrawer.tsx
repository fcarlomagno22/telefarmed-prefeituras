...
import {
  PROFISSIONAL_PASSWORD_RECOVERY_CODE_LENGTH,
  type ProfissionalPasswordRecoveryStepId,
} from '../../../config/profissionalPasswordRecovery'
import {
  profissionalCompletePasswordRecovery,
  profissionalRequestPasswordRecovery,
  profissionalVerifyPasswordRecoveryCode,
  ProfissionalPasswordRecoveryError,
} from '../../../lib/services/profissional/passwordRecovery'
...
import { ProfissionalPasswordRecoveryStepper } from './ProfissionalPasswordRecoveryStepper'
...
type ProfissionalPasswordRecoveryDrawerProps = {
  open: boolean
  closing: boolean
  onClose: () => void
...
export function ProfissionalPasswordRecoveryDrawer({
  open,
  closing,
  onClose,
  onTransitionEnd,
}: ProfissionalPasswordRecoveryDrawerProps) {
  ...
  return createPortal(
    ...
    >
      ...
      <aside
        role="dialog"
        aria-modal="true"
        aria-labelledby="profissional-password-recovery-title"
      ...
      >
        {isActive ? (
          <ProfissionalPasswordRecoveryDrawerContent key={sessionKey} onClose={onClose} />
        ) : null}
      </aside>
    </div>,
    document.body,
...
type ProfissionalPasswordRecoveryDrawerContentProps = {
  onClose: () => void
}

function ProfissionalPasswordRecoveryDrawerContent({ onClose }: ProfissionalPasswordRecoveryDrawerContentProps) {
  const [step, setStep] = useState<ProfissionalPasswordRecoveryStepId>('cpf')
  ...
  async function handleCpfSubmit(event: FormEvent) {
    ...
    try {
      const result = await profissionalRequestPasswordRecovery(cpf)
      ...
      setSubmitError(
        error instanceof ProfissionalPasswordRecoveryError
          ? error.message
  ...
  async function handleCodeSubmit(event: FormEvent) {
    ...
    if (code.length !== PROFISSIONAL_PASSWORD_RECOVERY_CODE_LENGTH || !resetToken) return

    setIsSubmitting(true)
    try {
      const result = await profissionalVerifyPasswordRecoveryCode({ resetToken, code })
  ...
  async function handlePasswordSubmit(event: FormEvent) {
    ...
    setIsSubmitting(true)
    try {
      await profissionalCompletePasswordRecovery({ verificationToken, password })
      setStep('success')
    } catch (error) {
      setSubmitError(
        error instanceof ProfissionalPasswordRecoveryError
  ...
  const stepSubtitle =
    step === 'cpf'
      ? 'Informe o CPF do profissional de saúde cadastrado.'
      : step === 'code'
        ? `Digite o código de ${PROFISSIONAL_PASSWORD_RECOVERY_CODE_LENGTH} dígitos enviado para ${sentToEmail}.`
        : 'Defina uma nova senha para acessar o portal profissional.'

  return (
    <div className="flex min-h-0 flex-1 flex-col">
      <header className="flex shrink-0 items-center justify-between px-6 pt-6">
        {showChrome ? (
          <h2 id="profissional-password-recovery-title" className="text-lg font-semibold text-gray-900">
            Recuperar senha
          </h2>
        ) : (
      ...
      {showStepper ? <ProfissionalPasswordRecoveryStepper currentStep={step} /> : null}
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-6 py-6">
          ...
          {step === 'code' ? (
            <form className="space-y-5" onSubmit={handleCodeSubmit} noValidate>
              <PinInput
                id="profissional-recovery-code"
                label={`Código de ${PROFISSIONAL_PASSWORD_RECOVERY_CODE_LENGTH} dígitos`}
                value={code}
                onChange={setCode}
                length={PROFISSIONAL_PASSWORD_RECOVERY_CODE_LENGTH}
                visible={showCode}
              ...
              <button
                type="submit"
                disabled={isSubmitting || code.length !== PROFISSIONAL_PASSWORD_RECOVERY_CODE_LENGTH}
                className={primaryButtonClass}
              >
