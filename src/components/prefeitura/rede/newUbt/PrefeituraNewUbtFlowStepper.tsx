...
import { newUbtFlowSteps, resolveNewUbtStepIndex, type NewUbtFormStep } from './newUbtFormTypes'

type PrefeituraNewUbtFlowStepperProps = {
  step: NewUbtFormStep
}

export function PrefeituraNewUbtFlowStepper({ step }: PrefeituraNewUbtFlowStepperProps) {
  const activeIndex = resolveNewUbtStepIndex(step)
  const [fillRatio, setFillRatio] = useState(0)
  const fillRatioRef = useRef(0)

  const total = newUbtFlowSteps.length
  const stepNumber = activeIndex + 1
  const currentLabel = newUbtFlowSteps[activeIndex]?.label ?? ''
  ...
  return (
    <nav
      aria-label="Progresso do cadastro de UBT"
    ...
    >
      ...
      >
        {newUbtFlowSteps.map((flowStep, index) => {
