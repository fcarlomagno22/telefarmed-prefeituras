...
import {
  prefeituraContratoOptions,
  type PrefeituraContratoStatus,
...
export function PrefeituraContratoSelector({
  ...
  const selected =
    prefeituraContratoOptions.find((option) => option.id === selectedId) ??
    prefeituraContratoOptions[0]
  ...
  return (
    <div ref={containerRef} className="relative shrink-0">
      <button
        ...
        className={[
          'inline-flex max-w-[min(100%,20rem)] items-center gap-2.5 rounded-xl border bg-white px-3.5 py-2.5 text-left shadow-sm transition',
          open
      ...
      {open ? (
        <div
          ...
          className="absolute right-0 top-[calc(100%+0.375rem)] z-[120] w-[min(100vw-2.5rem,22rem)] overflow-hidden rounded-xl border border-gray-200/90 bg-white shadow-[0_8px_24px_rgba(0,0,0,0.12)]"
        >
          ...
          <ul className="max-h-[16rem] overflow-y-auto py-1">
            {prefeituraContratoOptions.map((option) => {
