...
} from '../../../config/prefeituraPasswordRecovery'
import {
  prefeituraCompletePasswordRecovery,
  prefeituraRequestPasswordRecovery,
  prefeituraVerifyPasswordRecoveryCode,
  PrefeituraPasswordRecoveryError,
} from '../../../lib/services/prefeitura/passwordRecovery'
...
type PrefeituraPasswordRecoveryDrawerProps = {
  open: boolean
  closing: boolean
  onClose: () => void
...
export function PrefeituraPasswordRecoveryDrawer({
  open,
  closing,
  onClose,
  onTransitionEnd,
}: PrefeituraPasswordRecoveryDrawerProps) {
  ...
  return createPortal(
    ...
    >
      ...
      <aside
        role="dialog"
        aria-modal="true"
        aria-labelledby="prefeitura-password-recovery-title"
      ...
      >
        {isActive ? (
          <PrefeituraPasswordRecoveryDrawerContent key={sessionKey} onClose={onClose} />
        ) : null}
      </aside>
    </div>,
    document.body,
...
type PrefeituraPasswordRecoveryDrawerContentProps = {
  onClose: () => void
}
...
function PrefeituraPasswordRecoveryDrawerContent({ onClose }: PrefeituraPasswordRecoveryDrawerContentProps) {
  ...
  async function handleCpfSubmit(event: FormEvent) {
    ...
    try {
      const result = await prefeituraRequestPasswordRecovery(cpf)
      ...
      setSubmitError(
        error instanceof PrefeituraPasswordRecoveryError
          ? error.message
  ...
  async function handleCodeSubmit(event: FormEvent) {
    ...
    if (code.length !== PREFEITURA_PASSWORD_RECOVERY_CODE_LENGTH || !resetToken) return

    setIsSubmitting(true)
    try {
      const result = await prefeituraVerifyPasswordRecoveryCode({ resetToken, code })
  ...
  async function handlePasswordSubmit(event: FormEvent) {
    ...
    setIsSubmitting(true)
    try {
      await prefeituraCompletePasswordRecovery({ verificationToken, password })
      setStep('success')
    } catch (error) {
      setSubmitError(
        error instanceof PrefeituraPasswordRecoveryError
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      <header className="flex shrink-0 items-center justify-between px-6 pt-6">
        {showChrome ? (
          <h2 id="prefeitura-password-recovery-title" className="text-lg font-semibold text-gray-900">
            Recuperar senha
          </h2>
        ) : (
      ...
      {showStepper ? <PrefeituraPasswordRecoveryStepper currentStep={step} /> : null}
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-6 py-6">
          ...
          {step === 'code' ? (
            <form className="space-y-5" onSubmit={handleCodeSubmit} noValidate>
              <PinInput
                id="prefeitura-recovery-code"
                label={`Código de ${PREFEITURA_PASSWORD_RECOVERY_CODE_LENGTH} dígitos`}
                value={code}
                onChange={setCode}
                length={PREFEITURA_PASSWORD_RECOVERY_CODE_LENGTH}
                visible={showCode}
              ...
              <button
                type="submit"
                disabled={isSubmitting || code.length !== PREFEITURA_PASSWORD_RECOVERY_CODE_LENGTH}
                className={primaryButtonClass}
              >
