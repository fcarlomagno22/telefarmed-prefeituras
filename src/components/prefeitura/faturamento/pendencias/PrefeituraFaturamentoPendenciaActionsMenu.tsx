...
export type PrefeituraFaturamentoPendenciaMenuAction =
  | 'primary'
  | 'revisar_regra_sus'
  | 'view_consulta'
  | 'revalidar'
...
type PrefeituraFaturamentoPendenciaActionsMenuProps = {
  item: PrefeituraFaturamentoPendencia
  open: boolean
  onToggle: () => void
  onClose: () => void
  onAction: (action: PrefeituraFaturamentoPendenciaMenuAction) => void
...
const primaryActionIcon: Record<PrefeituraFaturamentoPendenciaAcao, LucideIcon> = {
  corrigir_cadastro: UserPen,
  editar_profissional: Stethoscope,
  abrir_consulta: ExternalLink,
  definir_procedimento: FileSearch,
  revisar_regra_sus: ShieldAlert,
  comparar_consultas: GitCompare,
  solicitar_ajuste_profissional: MessageSquare,
}
...
export function PrefeituraFaturamentoPendenciaActionsMenu({
  item,
  open,
  onToggle,
  onClose,
  onAction,
  align = 'center',
}: PrefeituraFaturamentoPendenciaActionsMenuProps) {
  const triggerRef = useRef<HTMLButtonElement>(null)
  const menuRef = useRef<HTMLDivElement>(null)
  const [menuStyle, setMenuStyle] = useState<CSSProperties | null>(null)

  const PrimaryIcon = primaryActionIcon[item.primaryAction]
  const showRevalidar = isPendenciaAberta(item.status)
  const showIgnore = canIgnorePendencia(item)
  ...
  useLayoutEffect(() => {
    ...
    function updatePosition() {
      ...
      // Instâncias ocultas (ex.: cards mobile com lg:hidden) ainda montam o componente
      // e gerariam um segundo portal fixo no canto superior esquerdo.
  ...
  }, [open, showIgnore, showRevalidar])

  useEffect(() => {
    // Instâncias ocultas (lg:hidden / hidden lg:block) ficam com open=true mas sem portal;
    // sem este guard, o listener trata clique no menu visível como "fora" e cancela a ação.
  ...
  function handleSelect(action: PrefeituraFaturamentoPendenciaMenuAction) {
    onAction(action)
    onClose()
  }
  ...
  return (
    <div
      ...
      data-pendencia-menu-root="true"
      onClick={(event) => event.stopPropagation()}
    >
      <button
        ref={triggerRef}
        type="button"
        title={`Ações — ${item.title}`}
        onClick={(event) => {
          event.stopPropagation()
          onToggle()
        }}
        ...
        aria-haspopup="menu"
        aria-expanded={open}
        aria-label={`Ações da pendência ${item.title}`}
      >
        ...
        ? createPortal(
            ...
            >
              ...
              <div className="py-1.5">
                <MenuItem
                  icon={PrimaryIcon}
                  label={prefeituraFaturamentoPendenciaAcaoLabel[item.primaryAction]}
                  tone="primary"
                  onClick={() => handleSelect('primary')}
                />
                {item.primaryAction !== 'revisar_regra_sus' ? (
                  <MenuItem
                    icon={ShieldAlert}
                    label={prefeituraFaturamentoPendenciaAcaoLabel.revisar_regra_sus}
                    onClick={() => handleSelect('revisar_regra_sus')}
                  />
                ) : null}
                <MenuItem
                  icon={Eye}
                ...
                {showRevalidar ? (
                  <MenuItem
                    icon={RefreshCw}
                    label="Revalidar pendência"
                    onClick={() => handleSelect('revalidar')}
                  />
                ) : null}
                {showIgnore ? (
                  <MenuItem
                    icon={AlertTriangle}
                    label="Ignorar com justificativa"
                    tone="warning"
                    onClick={() => handleSelect('ignore')}
                  />
                ) : null}
              </div>
            </div>,
