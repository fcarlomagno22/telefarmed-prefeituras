import { Eye, FileDown, FileText, MoreVertical, type LucideIcon } from 'lucide-react'
import { useEffect, useLayoutEffect, useRef, useState, type CSSProperties } from 'react'
import { createPortal } from 'react-dom'
import { FLOATING_POPOVER_Z_INDEX } from '../../../../config/overlayLayers'
import type { PrefeituraFaturamentoHistoricoItem } from '../../../../hooks/usePrefeituraFaturamentoFechamentoPage'

export type PrefeituraFaturamentoHistoricoMenuAction =
  | 'view_fechamento'
  | 'export_bpa_txt'
  | 'download_relatorio'

type PrefeituraFaturamentoHistoricoActionsMenuProps = {
  item: PrefeituraFaturamentoHistoricoItem
  open: boolean
  onToggle: () => void
  onClose: () => void
  onAction: (action: PrefeituraFaturamentoHistoricoMenuAction) => void
...
function MenuItem({ icon: Icon, label, onClick, tone = 'default' }: MenuItemProps) {
  ...
  const iconClass = tone === 'primary' ? 'text-[var(--brand-primary)]' : 'text-gray-400'
...
export function PrefeituraFaturamentoHistoricoActionsMenu({
  item,
  open,
  onToggle,
  onClose,
  onAction,
  align = 'center',
}: PrefeituraFaturamentoHistoricoActionsMenuProps) {
  ...
  function handleSelect(action: PrefeituraFaturamentoHistoricoMenuAction) {
    onAction(action)
    onClose()
  }
  ...
  return (
    <div
      ...
      data-fechamento-historico-menu-root="true"
      onClick={(event) => event.stopPropagation()}
    >
      <button
        ref={triggerRef}
        type="button"
        title={`Ações — ${item.competenciaLabel}`}
        onClick={(event) => {
          event.stopPropagation()
          onToggle()
        }}
        ...
        aria-haspopup="menu"
        aria-expanded={open}
        aria-label={`Ações do fechamento ${item.competenciaLabel}`}
      >
        ...
        ? createPortal(
            ...
            >
              <div className="border-b border-gray-100 bg-gradient-to-br from-slate-50 via-white to-[var(--brand-primary-light)]/40 px-3.5 py-3">
                ...
                <p className="mt-1 text-sm font-bold text-gray-900">{item.competenciaLabel}</p>
                <p className="mt-0.5 text-xs font-semibold text-violet-700">{item.tipoLabel}</p>
                <p className="mt-0.5 truncate font-mono text-xs text-gray-500">
                  {item.record.loteId ?? item.record.competencia}
                </p>
              </div>
              ...
              <div className="py-1.5">
                <MenuItem
                  icon={Eye}
                  label="Ver no fechamento"
                  tone="primary"
                  onClick={() => handleSelect('view_fechamento')}
                />
                <MenuItem
                  icon={FileText}
                  label="Exportar BPA (TXT)"
                  onClick={() => handleSelect('export_bpa_txt')}
                />
                <MenuItem
                  icon={FileDown}
                  label="Baixar relatório"
                  onClick={() => handleSelect('download_relatorio')}
                />
              </div>
            </div>,
            document.body,
