...
type Props = {
  report: NovosCadastrosReportApi
  brandName: string
  logoUrl: string
  ...
  return (
    <div className="mt-4">
          ...
          return (
            <div key={point.date ?? point.label} className="flex min-w-0 flex-1 flex-col items-center gap-2">
              ...
              <div
                ...
                style={{ height: barHeight, background: 'linear-gradient(to top, #4f46e5, #818cf8)' }}
              />
      ...
      <p className="mt-3 text-xs text-gray-500">
        Evolução {mode === 'monthly' ? 'mensal' : 'diária'} de novos cadastros na rede municipal.
      </p>
    </div>
  )
}

export function PrefeituraNovosCadastrosReportDocument({
  report,
  brandName,
  logoUrl,
  generatedAtLabel,
    ...
    <div className="p-6 sm:p-8">
      ...
      <section className="mt-8 grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <article className="rounded-xl border border-gray-200 bg-slate-50/70 p-4 text-center">
          <p className="text-xs font-medium text-gray-500">Novos cadastros</p>
          <p className="mt-1 text-2xl font-bold tabular-nums text-gray-900">
            {formatNumber(report.summary.newRegistrations)}
          </p>
        </article>
        <article className="rounded-xl border border-gray-200 bg-slate-50/70 p-4 text-center">
          <p className="text-xs font-medium text-gray-500">Variação</p>
          <p className="mt-1 text-2xl font-bold tabular-nums text-emerald-700">
            +{formatPercent(report.summary.registrationsDeltaPercent)}%
          </p>
        </article>
        <article className="rounded-xl border border-gray-200 bg-slate-50/70 p-4 text-center">
          <p className="text-xs font-medium text-gray-500">Média por dia</p>
          <p className="mt-1 text-2xl font-bold tabular-nums text-gray-900">{formatPercent(report.summary.avgPerDay)}</p>
        </article>
        <article className="rounded-xl border border-gray-200 bg-slate-50/70 p-4 text-center">
          <p className="text-xs font-medium text-gray-500">Canais / unidades</p>
          <p className="mt-1 text-2xl font-bold tabular-nums text-gray-900">
            {report.summary.channelsCount} / {report.summary.unitsCount}
          </p>
        </article>
      </section>
      ...
      <section className="mt-8">
        <h2 className="border-b-2 border-[var(--brand-primary)] pb-2 text-sm font-bold text-gray-900">Canais de cadastramento</h2>
        <div className="mt-4 overflow-x-auto">
          <table className="w-full min-w-[32rem] border-collapse text-left text-xs">
            <thead>
              <tr className="bg-gray-50 text-[11px] font-semibold uppercase tracking-wide text-gray-500">
                <th className="border border-gray-200 px-3 py-2.5 text-left">Canal</th>
                <th className={tableMetricCellClass}>Cadastros</th>
      ...
      <section className="mt-8">
        <h2 className="border-b-2 border-[var(--brand-primary)] pb-2 text-sm font-bold text-gray-900">Unidades com novos cadastros</h2>
        <div className="mt-4 overflow-x-auto">
      ...
      <section className="mt-8">
        <h2 className="border-b-2 border-[var(--brand-primary)] pb-2 text-sm font-bold text-gray-900">Evolução de cadastros</h2>
        <EvolutionChart points={report.evolution.registrationPoints} mode={report.evolution.mode} />
      </section>
