...
import {
  ...
  type AgendaComparecimentoReportApi,
} from '../../../lib/services/prefeitura/relatorios'
import { exportPrefeituraAgendaComparecimentoReportPdf } from '../../../utils/prefeitura/prefeituraAgendaComparecimentoReportExport'
import { formatDatePtBr } from '../../../utils/calendar'
import { PrefeituraAgendaComparecimentoReportDocument } from './PrefeituraAgendaComparecimentoReportDocument'
...
export function PrefeituraAgendaComparecimentoReportView() {
  const { getAccessToken } = usePrefeituraAuth()
  const [searchParams] = useSearchParams()
  const [report, setReport] = useState<AgendaComparecimentoReportApi | null>(null)
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        const data = await fetchPrefeituraAgendaComparecimentoReport(token, {
          periodStart,
          periodEnd,
        })
  ...
  const handleDownloadPdf = useCallback(async () => {
    ...
    try {
      await exportPrefeituraAgendaComparecimentoReportPdf({
        report,
    ...
    <div className="min-h-dvh bg-slate-50">
      ...
      <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6 sm:py-8">
        <div className="rounded-2xl border border-gray-200 bg-white shadow-[0_1px_3px_rgba(0,0,0,0.08),0_2px_10px_rgba(0,0,0,0.05)]">
          <PrefeituraAgendaComparecimentoReportDocument
            report={report}
