...
type Props = { report: InterrupcoesReconexoesReportApi; brandName: string; logoUrl: string; generatedAtLabel: string }
...
export function PrefeituraInterrupcoesReconexoesReportDocument({ report, brandName, logoUrl, generatedAtLabel }: Props) {
  return (
    <div className="p-6 sm:p-8">
      ...
      <section className="mt-8 grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <article className="rounded-xl border border-gray-200 bg-slate-50/70 p-4 text-center"><p className="text-xs font-medium text-gray-500">Taxa de interrupção</p><p className="mt-1 text-2xl font-bold tabular-nums text-gray-900">{formatPercent(report.summary.interruptionRatePercent)}%</p><p className="mt-1 text-xs font-medium text-emerald-600">{signedPp(report.summary.interruptionDeltaPp)} vs período anterior</p></article>
        <article className="rounded-xl border border-gray-200 bg-slate-50/70 p-4 text-center"><p className="text-xs font-medium text-gray-500">Reconexões</p><p className="mt-1 text-2xl font-bold tabular-nums text-gray-900">{formatNumber(report.summary.reconnections)}</p><p className="mt-1 text-xs text-gray-500">{formatPercent(report.summary.reconnectionRatePercent)}% das consultas</p></article>
        <article className="rounded-xl border border-gray-200 bg-slate-50/70 p-4 text-center"><p className="text-xs font-medium text-gray-500">Conclusão da rede</p><p className="mt-1 text-2xl font-bold tabular-nums text-gray-900">{formatPercent(report.summary.completionRatePercent)}%</p><p className="mt-1 text-xs font-medium text-emerald-600">{signedPp(report.summary.completionDeltaPp)} vs período anterior</p></article>
        <article className="rounded-xl border border-gray-200 bg-slate-50/70 p-4 text-center"><p className="text-xs font-medium text-gray-500">Consultas analisadas</p><p className="mt-1 text-2xl font-bold tabular-nums text-gray-900">{formatNumber(report.summary.totalConsultas)}</p></article>
      </section>
      <HighlightsSection highlights={report.highlights} />
      <section className="mt-8"><h2 className="border-b-2 border-[var(--brand-primary)] pb-2 text-sm font-bold text-gray-900">Composição das ocorrências</h2><div className="mt-4 overflow-x-auto"><table className="w-full min-w-[40rem] border-collapse text-left text-xs"><thead><tr className="bg-gray-50 text-[11px] font-semibold uppercase tracking-wide text-gray-500"><th className="border border-gray-200 px-3 py-2.5 text-left">Tipo</th><th className={tableMetricCellClass}>Volume</th><th className={tableMetricCellClass}>Participação</th><th className={tableMetricCellClass}>Taxa conclusão</th></tr></thead><tbody>{report.breakdown.map((row) => (<tr key={row.key} className="text-gray-800 even:bg-gray-50/60"><td className="border border-gray-200 px-3 py-2.5 font-semibold">{row.label}</td><td className={[tableMetricCellClass, 'font-bold tabular-nums'].join(' ')}>{formatNumber(row.count)}</td><td className={[tableMetricCellClass, 'tabular-nums'].join(' ')}>{formatPercent(row.sharePercent)}%</td><td className={[tableMetricCellClass, 'tabular-nums'].join(' ')}>{formatPercent(row.completionRatePercent)}%</td></tr>))}</tbody></table></div></section>
      <section className="mt-8"><h2 className="border-b-2 border-[var(--brand-primary)] pb-2 text-sm font-bold text-gray-900">Comparativo entre unidades</h2><div className="mt-4 overflow-x-auto"><table className="w-full min-w-[52rem] border-collapse text-left text-xs"><thead><tr className="bg-gray-50 text-[11px] font-semibold uppercase tracking-wide text-gray-500"><th className="border border-gray-200 px-3 py-2.5 text-left">Unidade</th><th className={tableMetricCellClass}>Região</th><th className={tableMetricCellClass}>Consultas</th><th className={tableMetricCellClass}>Interrupções</th><th className={tableMetricCellClass}>Reconexões</th><th className={tableMetricCellClass}>Taxa interrupção</th><th className={tableMetricCellClass}>vs rede</th></tr></thead><tbody>{report.units.map((row) => (<tr key={row.id} className="text-gray-800 even:bg-gray-50/60"><td className="border border-gray-200 px-3 py-2.5 font-semibold">{row.name}</td><td className={tableMetricCellClass}>{row.region}</td><td className={[tableMetricCellClass, 'tabular-nums'].join(' ')}>{formatNumber(row.totalConsultas)}</td><td className={[tableMetricCellClass, 'tabular-nums'].join(' ')}>{formatNumber(row.interruptions)}</td><td className={[tableMetricCellClass, 'tabular-nums'].join(' ')}>{formatNumber(row.reconnections)}</td><td className={[tableMetricCellClass, 'font-bold tabular-nums'].join(' ')}>{formatPercent(row.interruptionRatePercent)}%</td><td className={[tableMetricCellClass, 'tabular-nums font-medium', row.completionVsNetworkPp >= 0 ? 'text-emerald-700' : 'text-amber-700'].join(' ')}>{signedPp(row.completionVsNetworkPp)}</td></tr>))}</tbody></table></div></section>
      <section className="mt-8"><h2 className="border-b-2 border-[var(--brand-primary)] pb-2 text-sm font-bold text-gray-900">Evolução das interrupções</h2><EvolutionChart points={report.evolution.interruptionPoints} mode={report.evolution.mode} title="Interrupções" barStyle="linear-gradient(to top, #dc2626, #f87171)" /></section>
      <section className="mt-8"><h2 className="border-b-2 border-[var(--brand-primary)] pb-2 text-sm font-bold text-gray-900">Evolução das reconexões</h2><EvolutionChart points={report.evolution.reconnectionPoints} mode={report.evolution.mode} title="Reconexões" barStyle="linear-gradient(to top, #ea580c, #fb923c)" /></section>
      <section className="mt-8"><h2 className="border-b-2 border-[var(--brand-primary)] pb-2 text-sm font-bold text-gray-900">Evolução da taxa de conclusão</h2><EvolutionChart points={report.evolution.completionRatePoints} mode={report.evolution.mode} title="Taxa de conclusão" valueSuffix="%" barStyle="linear-gradient(to top, #059669, #34d399)" /></section>
