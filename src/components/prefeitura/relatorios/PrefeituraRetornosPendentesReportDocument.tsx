...
export function PrefeituraRetornosPendentesReportDocument({
  report,
  brandName,
  logoUrl,
  generatedAtLabel,
    ...
    <div className="p-6 sm:p-8">
      ...
      <section className="mt-8 grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <article className="rounded-xl border border-gray-200 bg-slate-50/70 p-4 text-center">
          <p className="text-xs font-medium text-gray-500">Retornos pendentes</p>
          <p className="mt-1 text-2xl font-bold tabular-nums text-gray-900">
            {formatNumber(report.summary.pendingCount)}
          </p>
        </article>
        <article className="rounded-xl border border-gray-200 bg-slate-50/70 p-4 text-center">
          <p className="text-xs font-medium text-gray-500">Não agendados</p>
          <p className="mt-1 text-2xl font-bold tabular-nums text-amber-700">
            {formatNumber(report.summary.notScheduledCount)}
          </p>
        </article>
        <article className="rounded-xl border border-gray-200 bg-slate-50/70 p-4 text-center">
          <p className="text-xs font-medium text-gray-500">Não realizados</p>
          <p className="mt-1 text-2xl font-bold tabular-nums text-rose-700">
            {formatNumber(report.summary.notPerformedCount)}
          </p>
        </article>
        <article className="rounded-xl border border-gray-200 bg-slate-50/70 p-4 text-center">
          <p className="text-xs font-medium text-gray-500">Atrasados críticos</p>
          <p className="mt-1 text-2xl font-bold tabular-nums text-violet-700">
            {formatNumber(report.summary.overdueCount)}
          </p>
        </article>
      </section>
      ...
      <section className="mt-8">
        <h2 className="border-b-2 border-[var(--brand-primary)] pb-2 text-sm font-bold text-gray-900">Quebra por tipo de pendência</h2>
      ...
      <section className="mt-8">
        <h2 className="border-b-2 border-[var(--brand-primary)] pb-2 text-sm font-bold text-gray-900">Unidades com retorno pendente</h2>
        <div className="mt-4 overflow-x-auto">
      ...
      <section className="mt-8">
        <h2 className="border-b-2 border-[var(--brand-primary)] pb-2 text-sm font-bold text-gray-900">Pacientes com pendências prioritárias</h2>
        <div className="mt-4 overflow-x-auto">
          <table className="w-full min-w-[44rem] border-collapse text-left text-xs">
            <thead>
              <tr className="bg-gray-50 text-[11px] font-semibold uppercase tracking-wide text-gray-500">
                ...
                <th className={tableMetricCellClass}>Situação</th>
                <th className={tableMetricCellClass}>Data prevista</th>
                <th className={tableMetricCellClass}>Dias em atraso</th>
              </tr>
            </thead>
            <tbody>
              {report.patients.map((row) => (
                <tr key={row.id} className="text-gray-800 even:bg-gray-50/60">
                  <td className="border border-gray-200 px-3 py-2.5 font-semibold">{row.patientName}</td>
                  <td className={tableMetricCellClass}>{row.unitName}</td>
                  <td className={tableMetricCellClass}>{row.status}</td>
                  <td className={tableMetricCellClass}>{row.scheduledDate}</td>
                  <td className={tableMetricCellClass}>{row.daysOverdue}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <section className="mt-8">
        <h2 className="border-b-2 border-[var(--brand-primary)] pb-2 text-sm font-bold text-gray-900">Evolução dos pendentes</h2>
        <EvolutionChart points={report.evolution.pendingPoints} mode={report.evolution.mode} />
      </section>
