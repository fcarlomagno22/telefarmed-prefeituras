...
import {
  ...
  type ProducaoUnidadeReportApi,
} from '../../../lib/services/prefeitura/relatorios'
import { exportPrefeituraProducaoUnidadeReportPdf } from '../../../utils/prefeitura/prefeituraProducaoUnidadeReportExport'
import { formatDatePtBr } from '../../../utils/calendar'
import { prefeituraSlaBadgeConfig } from '../prefeituraDashboardUi'
import { PrefeituraProducaoUnidadeReportDocument } from './PrefeituraProducaoUnidadeReportDocument'
...
export function PrefeituraProducaoUnidadeReportView() {
  const { getAccessToken } = usePrefeituraAuth()
  const [searchParams] = useSearchParams()
  const [report, setReport] = useState<ProducaoUnidadeReportApi | null>(null)
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        const data = await fetchPrefeituraProducaoUnidadeReport(token, {
          periodStart,
          periodEnd,
        })
  ...
  const handleDownloadPdf = useCallback(async () => {
    ...
    try {
      await exportPrefeituraProducaoUnidadeReportPdf({
        report,
    ...
    <div className="min-h-dvh bg-slate-50">
      ...
      <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6 sm:py-8">
        <div className="rounded-2xl border border-gray-200 bg-white shadow-[0_1px_3px_rgba(0,0,0,0.08),0_2px_10px_rgba(0,0,0,0.05)]">
          <PrefeituraProducaoUnidadeReportDocument
            report={report}
