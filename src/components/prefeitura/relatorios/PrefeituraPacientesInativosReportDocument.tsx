          ...
          return (
            <div key={point.date ?? point.label} className="flex min-w-0 flex-1 flex-col items-center gap-2">
              ...
              <div
                ...
                style={{ height: barHeight, background: 'linear-gradient(to top, #ea580c, #fb923c)' }}
              />
...
export function PrefeituraPacientesInativosReportDocument({
  report,
  brandName,
  logoUrl,
  generatedAtLabel,
    ...
    <div className="p-6 sm:p-8">
      ...
      <section className="mt-8 grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <article className="rounded-xl border border-gray-200 bg-slate-50/70 p-4 text-center">
          <p className="text-xs font-medium text-gray-500">Pacientes inativos</p>
          <p className="mt-1 text-2xl font-bold tabular-nums text-gray-900">
            {formatNumber(report.summary.inactiveCount)}
          </p>
        </article>
        <article className="rounded-xl border border-gray-200 bg-slate-50/70 p-4 text-center">
          <p className="text-xs font-medium text-gray-500">Taxa de inatividade</p>
          <p className="mt-1 text-2xl font-bold tabular-nums text-amber-700">
            {formatPercent(report.summary.inactiveRatePercent)}%
          </p>
        </article>
        <article className="rounded-xl border border-gray-200 bg-slate-50/70 p-4 text-center">
          <p className="text-xs font-medium text-gray-500">Nunca consultaram</p>
          <p className="mt-1 text-2xl font-bold tabular-nums text-rose-700">
            {formatNumber(report.summary.neverConsultedCount)}
          </p>
        </article>
        <article className="rounded-xl border border-gray-200 bg-slate-50/70 p-4 text-center">
      ...
      <section className="mt-8">
        <h2 className="border-b-2 border-[var(--brand-primary)] pb-2 text-sm font-bold text-gray-900">Faixas de inatividade</h2>
        <div className="mt-4 overflow-x-auto">
          <table className="w-full min-w-[32rem] border-collapse text-left text-xs">
            <thead>
              <tr className="bg-gray-50 text-[11px] font-semibold uppercase tracking-wide text-gray-500">
                <th className="border border-gray-200 px-3 py-2.5 text-left">Faixa</th>
      ...
      <section className="mt-8">
        <h2 className="border-b-2 border-[var(--brand-primary)] pb-2 text-sm font-bold text-gray-900">Unidades com pacientes inativos</h2>
        <div className="mt-4 overflow-x-auto">
      ...
      <section className="mt-8">
        <h2 className="border-b-2 border-[var(--brand-primary)] pb-2 text-sm font-bold text-gray-900">Evolução da inatividade</h2>
        <EvolutionChart points={report.evolution.inactivePoints} mode={report.evolution.mode} />
      </section>
