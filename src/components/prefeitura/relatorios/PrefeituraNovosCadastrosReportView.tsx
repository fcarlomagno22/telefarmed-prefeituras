...
import {
  ...
  type NovosCadastrosReportApi,
} from '../../../lib/services/prefeitura/relatorios'
import { exportPrefeituraNovosCadastrosReportPdf } from '../../../utils/prefeitura/prefeituraNovosCadastrosReportExport'
import { formatDatePtBr } from '../../../utils/calendar'
import { PrefeituraNovosCadastrosReportDocument } from './PrefeituraNovosCadastrosReportDocument'
...
export function PrefeituraNovosCadastrosReportView() {
  const { brandName, logoUrl } = useEntidadeReportPresentation()
  const { getAccessToken } = usePrefeituraAuth()
  const [searchParams] = useSearchParams()
  const [report, setReport] = useState<NovosCadastrosReportApi | null>(null)
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError(null)
      try {
        const data = await fetchPrefeituraNovosCadastrosReport(token, { periodStart, periodEnd })
  ...
  const handleDownloadPdf = useCallback(async () => {
    ...
    try {
      await exportPrefeituraNovosCadastrosReportPdf({
        report,
    ...
    <div className="min-h-dvh bg-slate-50">
      ...
      <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6 sm:py-8">
        <div className="rounded-2xl border border-gray-200 bg-white shadow-[0_1px_3px_rgba(0,0,0,0.08),0_2px_10px_rgba(0,0,0,0.05)]">
          <PrefeituraNovosCadastrosReportDocument
            report={report}
            brandName={brandName}
            logoUrl={logoUrl}
            generatedAtLabel={formatGeneratedAt(report.generatedAt)}
