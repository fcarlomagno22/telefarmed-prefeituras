...
import {
  ...
  type RetornosPendentesReportApi,
} from '../../../lib/services/prefeitura/relatorios'
import { exportPrefeituraRetornosPendentesReportPdf } from '../../../utils/prefeitura/prefeituraRetornosPendentesReportExport'
import { formatDatePtBr } from '../../../utils/calendar'
import { PrefeituraRetornosPendentesReportDocument } from './PrefeituraRetornosPendentesReportDocument'
...
export function PrefeituraRetornosPendentesReportView() {
  const { getAccessToken } = usePrefeituraAuth()
  const [searchParams] = useSearchParams()
  const [report, setReport] = useState<RetornosPendentesReportApi | null>(null)
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError(null)
      try {
        const data = await fetchPrefeituraRetornosPendentesReport(token, {
          periodStart,
          periodEnd,
        })
  ...
  const handleDownloadPdf = useCallback(async () => {
    ...
    try {
      await exportPrefeituraRetornosPendentesReportPdf({
        report,
    ...
    <div className="min-h-dvh bg-slate-50">
      ...
      <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6 sm:py-8">
        <div className="rounded-2xl border border-gray-200 bg-white shadow-[0_1px_3px_rgba(0,0,0,0.08),0_2px_10px_rgba(0,0,0,0.05)]">
          <PrefeituraRetornosPendentesReportDocument
            report={report}
