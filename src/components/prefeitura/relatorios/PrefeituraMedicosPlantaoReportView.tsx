...
import {
  ...
  type MedicosPlantaoReportApi,
} from '../../../lib/services/prefeitura/relatorios'
import { exportPrefeituraMedicosPlantaoReportPdf } from '../../../utils/prefeitura/prefeituraMedicosPlantaoReportExport'
import { formatDatePtBr } from '../../../utils/calendar'
import { PrefeituraMedicosPlantaoReportDocument } from './PrefeituraMedicosPlantaoReportDocument'
...
export function PrefeituraMedicosPlantaoReportView() {
  const { brandName, logoUrl } = useEntidadeReportPresentation()
  const { getAccessToken } = usePrefeituraAuth()
  const [searchParams] = useSearchParams()
  const [report, setReport] = useState<MedicosPlantaoReportApi | null>(null)
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        const data = await fetchPrefeituraMedicosPlantaoReport(token, {
          periodStart,
          periodEnd,
        })
  ...
  const handleDownloadPdf = useCallback(async () => {
    ...
    try {
      await exportPrefeituraMedicosPlantaoReportPdf({
        report,
    ...
    <div className="min-h-dvh bg-slate-50">
      ...
      <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6 sm:py-8">
        <div className="rounded-2xl border border-gray-200 bg-white shadow-[0_1px_3px_rgba(0,0,0,0.08),0_2px_10px_rgba(0,0,0,0.05)]">
          <PrefeituraMedicosPlantaoReportDocument
            report={report}
            brandName={brandName}
            logoUrl={logoUrl}
            generatedAtLabel={formatGeneratedAt(report.generatedAt)}
