...
type Props = { report: UnidadesCriticasReportApi; brandName: string; logoUrl: string; generatedAtLabel: string }
...
const SEVERITY_CLASS = { critico: 'text-rose-700 font-bold', atencao: 'text-amber-700 font-bold' } as const
export function PrefeituraUnidadesCriticasReportDocument({ report, brandName, logoUrl, generatedAtLabel }: Props) {
  return (
    <div className="p-6 sm:p-8">
      ...
      <section className="mt-8 grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <article className="rounded-xl border border-gray-200 bg-slate-50/70 p-4 text-center"><p className="text-xs font-medium text-gray-500">Unidades críticas</p><p className="mt-1 text-2xl font-bold tabular-nums text-gray-900">{formatNumber(report.summary.criticalUnitsCount)}</p><p className="mt-1 text-xs text-rose-600">Abaixo dos parâmetros mínimos</p></article>
        <article className="rounded-xl border border-gray-200 bg-slate-50/70 p-4 text-center"><p className="text-xs font-medium text-gray-500">Itens de ação</p><p className="mt-1 text-2xl font-bold tabular-nums text-gray-900">{formatNumber(report.summary.actionItemsCount)}</p><p className="mt-1 text-xs text-gray-500">Indicadores fora do limite</p></article>
        <article className="rounded-xl border border-gray-200 bg-slate-50/70 p-4 text-center"><p className="text-xs font-medium text-gray-500">Especialidades críticas</p><p className="mt-1 text-2xl font-bold tabular-nums text-gray-900">{formatNumber(report.summary.criticalSpecialtiesCount)}</p></article>
        <article className="rounded-xl border border-gray-200 bg-slate-50/70 p-4 text-center"><p className="text-xs font-medium text-gray-500">Rede monitorada</p><p className="mt-1 text-2xl font-bold tabular-nums text-gray-900">{formatNumber(report.summary.unitsCount)}</p><p className="mt-1 text-xs text-gray-500">Unidades na base de análise</p></article>
      </section>
      <HighlightsSection highlights={report.highlights} />
      <section className="mt-8"><h2 className="border-b-2 border-[var(--brand-primary)] pb-2 text-sm font-bold text-gray-900">Unidades abaixo dos parâmetros</h2><div className="mt-4 overflow-x-auto"><table className="w-full min-w-[52rem] border-collapse text-left text-xs"><thead><tr className="bg-gray-50 text-[11px] font-semibold uppercase tracking-wide text-gray-500"><th className="border border-gray-200 px-3 py-2.5 text-left">Unidade</th><th className={tableMetricCellClass}>Região</th><th className={tableMetricCellClass}>Severidade</th><th className={tableMetricCellClass}>Itens</th><th className={tableMetricCellClass}>Conclusão</th><th className={tableMetricCellClass}>Nota</th><th className={tableMetricCellClass}>Interrupção</th><th className={tableMetricCellClass}>NPS</th></tr></thead><tbody>{report.units.map((row) => (<tr key={row.id} className="text-gray-800 even:bg-gray-50/60"><td className="border border-gray-200 px-3 py-2.5 font-semibold">{row.name}</td><td className={tableMetricCellClass}>{row.region}</td><td className={[tableMetricCellClass, SEVERITY_CLASS[row.severity]].join(' ')}>{row.severity === 'critico' ? 'Crítico' : 'Atenção'}</td><td className={[tableMetricCellClass, 'font-bold tabular-nums'].join(' ')}>{formatNumber(row.issuesCount)}</td><td className={[tableMetricCellClass, 'tabular-nums'].join(' ')}>{formatPercent(row.completionRatePercent)}%</td><td className={[tableMetricCellClass, 'tabular-nums'].join(' ')}>{formatPercent(row.avgRating, 1)}</td><td className={[tableMetricCellClass, 'tabular-nums'].join(' ')}>{formatPercent(row.interruptionRatePercent)}%</td><td className={[tableMetricCellClass, 'tabular-nums'].join(' ')}>{formatPercent(row.nps, 1)}</td></tr>))}</tbody></table></div></section>
      <section className="mt-8"><h2 className="border-b-2 border-[var(--brand-primary)] pb-2 text-sm font-bold text-gray-900">Especialidades críticas</h2><div className="mt-4 overflow-x-auto"><table className="w-full min-w-[40rem] border-collapse text-left text-xs"><thead><tr className="bg-gray-50 text-[11px] font-semibold uppercase tracking-wide text-gray-500"><th className="border border-gray-200 px-3 py-2.5 text-left">Especialidade</th><th className={tableMetricCellClass}>Unidade</th><th className={tableMetricCellClass}>Severidade</th><th className={tableMetricCellClass}>Itens</th></tr></thead><tbody>{report.specialties.map((row) => (<tr key={row.id} className="text-gray-800 even:bg-gray-50/60"><td className="border border-gray-200 px-3 py-2.5 font-semibold">{row.name}</td><td className={tableMetricCellClass}>{row.unitName}</td><td className={[tableMetricCellClass, SEVERITY_CLASS[row.severity]].join(' ')}>{row.severity === 'critico' ? 'Crítico' : 'Atenção'}</td><td className={[tableMetricCellClass, 'font-bold tabular-nums'].join(' ')}>{formatNumber(row.issuesCount)}</td></tr>))}</tbody></table></div></section>
      <section className="mt-8"><h2 className="border-b-2 border-[var(--brand-primary)] pb-2 text-sm font-bold text-gray-900">Evolução de unidades críticas</h2><EvolutionChart points={report.evolution.criticalCountPoints} mode={report.evolution.mode} title="Unidades críticas" barStyle="linear-gradient(to top, #dc2626, #f87171)" /></section>
