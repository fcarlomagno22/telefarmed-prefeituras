...
import { prefeituraAgendasWeeklySummary } from '../../../data/prefeituraAgendasMock'
import {
...
export function PrefeituraAgendasSidebarPanel() {
  const packageUsage = useMemo(
    () => computePrefeituraPackageUsage(AGENDAS_PACKAGE_FILTERS, 1),
    [],
  )
  const packageStyles = prefeituraPackageStatusStyles[packageUsage.status]
  const usageBarWidth = Math.min(100, packageUsage.usagePercent)
    ...
    <aside className="flex shrink-0 flex-col gap-4">
      <section className="shrink-0 rounded-2xl border border-gray-200/90 bg-white p-4 shadow-[0_1px_2px_rgba(15,23,42,0.06),0_8px_24px_rgba(15,23,42,0.04)]">
        ...
        <ul className="mt-3 flex flex-col gap-2">
          {prefeituraAgendasWeeklySummary.map((item) => {
