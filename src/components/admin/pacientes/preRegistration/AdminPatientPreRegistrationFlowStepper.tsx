...
import {
  adminPatientPreRegistrationFlowSteps,
  resolveAdminPatientPreRegistrationStepIndex,
  type AdminPatientPreRegistrationStep,
} from './adminPatientPreRegistrationTypes'

type AdminPatientPreRegistrationFlowStepperProps = {
  step: AdminPatientPreRegistrationStep
}

export function AdminPatientPreRegistrationFlowStepper({
  step,
}: AdminPatientPreRegistrationFlowStepperProps) {
  const activeIndex = resolveAdminPatientPreRegistrationStepIndex(step)
  const [fillRatio, setFillRatio] = useState(0)
  const fillRatioRef = useRef(0)

  const total = adminPatientPreRegistrationFlowSteps.length
  const allDone = step === 'success'
  const stepNumber = allDone ? total : activeIndex + 1
  const currentLabel = allDone
    ? 'Pré-cadastro confirmado'
    : adminPatientPreRegistrationFlowSteps[activeIndex]?.label ?? ''
  ...
  return (
    ...
    >
      ...
      <div className="flex gap-1.5">
        {adminPatientPreRegistrationFlowSteps.map((flowStep, index) => {
