...
function buildCountSlices(
  ...
  getKey: (row: AdminInternoCredentialUser) => string,
): BarSlice[] {
...
type AdminCredenciaisInternoAboutPanelProps = {
  rows: AdminInternoCredentialUser[]
}

export function AdminCredenciaisInternoAboutPanel({ rows }: AdminCredenciaisInternoAboutPanelProps) {
  const total = rows.length
  const activeCount = rows.filter((row) => row.status === 'ativo').length
  const blockedCount = total - activeCount

  const departments = useMemo(
    () => buildCountSlices(rows, (row) => getDepartmentLabel(row.departmentId)),
    [rows],
  )
      ...
      <div className="flex min-h-0 flex-1 flex-col px-5 py-4 sm:px-6">
        ...
        <div className="mt-4 min-h-0 flex-1 space-y-3 overflow-y-auto pr-1">
          <section className="rounded-xl border border-gray-200 bg-gray-50/60 p-4">
            <h3 className="text-xs font-semibold uppercase tracking-wide text-gray-500">Por área</h3>
            <div className="mt-3">
              <HorizontalBars
                data={departments}
