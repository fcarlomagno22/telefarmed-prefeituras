...
import {
  ...
  inferPrefeituraAccessLevelFromPermissions,
  prefeituraPortalPages,
  type PrefeituraCredentialUser,
  type PrefeituraPortalPageId,
...
type AdminPrefeituraCredentialDrawerProps = {
  open: boolean
  closing: boolean
...
export function AdminPrefeituraCredentialDrawer({
  open,
  closing,
  mode,
  editingUser,
  ...
  onSave,
  isSaving = false,
}: AdminPrefeituraCredentialDrawerProps) {
  ...
  const [pagePermissions, setPagePermissions] = useState(emptyPrefeituraPagePermissions())
  ...
  const resetForm = useCallback(() => {
    ...
    if (editingUser) {
      ...
      setContractingEntityId(editingUser.contractingEntityId)
    ...
    setCpf('')
    setRole('')
    setContractingEntityId(contractingEntityOptions[0]?.value ?? '')
    setAccessLevel('operador')
    setPagePermissions(buildPrefeituraPagePermissions('operador'))
    setStatus('ativo')
  }, [contractingEntityOptions, editingUser])
  ...
  function applyAccessLevelPreset(level: AccessLevelId) {
    ...
    setPagePermissions(buildPrefeituraPagePermissions(level))
  }

  function togglePageAction(pageId: PrefeituraPortalPageId, action: PermissionAction) {
    setPagePermissions((prev) => {
      const next = {
        ...prev,
        [pageId]: toggleAction(prev[pageId], action),
      }
      setAccessLevel(inferPrefeituraAccessLevelFromPermissions(next))
      return next
    })
  }

  function togglePageEnabled(pageId: PrefeituraPortalPageId, enabled: boolean) {
    setPagePermissions((prev) => {
      const next = {
        ...prev,
  ...
  function handleSubmit(event: FormEvent) {
    ...
    const id = editingUser?.id ?? `pref-cred-${Date.now()}`
    ...
    const selectedOption = contractingEntityOptions.find(
      (option) => option.value === contractingEntityId,
    )
    const entityFromEditing = editingUser?.contractingEntity
    ...
    void onSave(
      {
        ...
        role: role.trim(),
        contractingEntityId,
        contractingEntity: entityFromEditing ?? {
          id: contractingEntityId,
          razaoSocial: selectedOption?.label.split(' · ')[0]?.trim() ?? 'Entidade',
          municipality: selectedOption?.label.split(' · ')[1]?.split('/')[0]?.trim() ?? '',
          uf: selectedOption?.label.split('/').pop()?.trim() ?? 'SP',
        },
        accessLevel,
        status,
  ...
  const canSubmit =
    ...
    cpfValid &&
    role.trim().length > 0 &&
    contractingEntityId.length > 0 &&
    prefeituraPortalPages.some((page) => pagePermissions[page.id].length > 0) &&
    passwordValid &&
    pinValid
  ...
  return createPortal(
    ...
    >
      ...
      <aside
        role="dialog"
        aria-modal="true"
        aria-labelledby="admin-prefeitura-drawer-title"
        ...
        <form onSubmit={handleSubmit} className="flex min-h-0 flex-1 flex-col">
          ...
          <div className="min-h-0 flex-1 space-y-6 overflow-y-auto px-5 py-5 sm:px-6">
            <section className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="sm:col-span-2">
                <label className="mb-1.5 block text-xs font-semibold text-gray-600">
                  Entidade contratante <span className="text-red-500">*</span>
                </label>
                {isViewMode || lockContractingEntity ? (
                  <div className={`${inputClass} bg-gray-50 text-gray-700`}>
                    {editingUser?.contractingEntity.razaoSocial ??
                      contractingEntityOptions.find((item) => item.value === contractingEntityId)
                        ?.label ??
                      contractingEntityOptions[0]?.label ??
                      '—'}
                  </div>
                ) : (
                  <CustomSelect
                    value={contractingEntityId}
                    onChange={setContractingEntityId}
                    options={contractingEntityOptions}
                    className="text-left"
                  />
                )}
              </div>
                    ...
                    <div className="mt-2">
                      {editingUser.hasPassword ? (
                        <p className="text-xs text-gray-500">
                          Este gestor já possui senha para entrar no portal (CPF + senha).
                        </p>
                      ) : (
                ...
                <section>
                  ...
                  {showPinInputs ? (
                    <div className="mt-4 grid grid-cols-1 gap-6 sm:grid-cols-2 sm:items-start sm:gap-x-12 lg:max-w-4xl xl:gap-x-20">
                      ...
                      />
                      <PinInput
                        id="admin-prefeitura-auth-pin-confirm"
            ...
            <section>
              ...
              <p className="mt-1 text-xs text-gray-500">
                Marque as páginas que o gestor pode acessar e o tipo de ação permitida em cada uma.
              </p>
              ...
              <div className="mt-3 overflow-hidden rounded-xl border border-gray-200">
                ...
                <ul className="divide-y divide-gray-100">
                  {prefeituraPortalPages.map((page) => {
